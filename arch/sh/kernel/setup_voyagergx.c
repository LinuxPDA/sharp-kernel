...
#undef VOYAGERGX_DEBUG

#ifdef VOYAGERGX_DEBUG
...
static void disable_voyagergx_irq(unsigned int irq)
{
	...
	unsigned long  mask = 1 << (irq - VOYAGER_IRQ_BASE);
    	...
    	DPRINTK("disable_voyagergx_irq(%d): mask=%x\n", irq, mask);
	save_and_cli(flags);
...
static void enable_voyagergx_irq(unsigned int irq)
{
        ...
        DPRINTK("enable_voyagergx_irq (%d): mask=%x\n", irq, mask);
        save_and_cli(flags);
        val = inl(VOYAGER_INT_MASK);
        val |= mask;
        outl(val, VOYAGER_INT_MASK);
...
static void voyagergx_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
	printk(KERN_INFO
...
int voyagergx_irq_demux(int irq)
{
	...
	if (irq == IRQ_VOYAGER ) {
		...
		if ( val & ( 1 << 1 )){
			...
			printk("Unexpected IRQ irq = %d status = 0x%08x\n", irq, val);	
		} 
		DPRINTK("voyagergx_irq_demux %d \n", i);
#else
...
void __init setup_voyagergx_irq(void)
{
	...
	for (i = 0; i < VOYAGER_IRQ_NUM ; i++) {
		...
		if (flag == 1)
			irq_desc[VOYAGER_IRQ_BASE + i].handler =
							&voyagergx_irq_type;
	}
