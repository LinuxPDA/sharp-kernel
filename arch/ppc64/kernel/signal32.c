...
long sys32_sigreturn(struct pt_regs *regs)
{
  ...
  PPCDBG(PPCDBG_SIGNAL, "sys32_sigreturn - comm=%s reg1 =%lx \n", 
         current->comm, regs->gpr[1]);
  ...
  {
    PPCDBG(PPCDBG_SIGNAL, "sys32_sigreturn - no more sigcontexts found \n");
  ...
  {
    PPCDBG(PPCDBG_SIGNAL, "sys32_sigreturn - additional sigcontexts found \n");
    /* More signals to go */
            ...
            unsigned int newsp)
{
  ...
  PPCDBG(PPCDBG_SIGNAL, "setup32 - entered - newsp =%lx \n", newsp);
  /***************************************************************/
  /*                                                             */ 
  ...
  PPCDBG(PPCDBG_SIGNAL, "setup32 - after copy to user \n");
  ...
  if (__copy_to_user(&frame->fp_regs, current->thread.fpr,
   ...
   PPCDBG(PPCDBG_SIGNAL, "setup32 - after copy to user2 \n");
  
  
  flush_icache_range((unsigned long) &frame->tramp[0],
                     (unsigned long) &frame->tramp[2]);

   PPCDBG(PPCDBG_SIGNAL, "setup32 - after flush icache range\n");
  ...
  if (put_user(regs->gpr[1], (u32*)(u64)newsp)
   ...
   PPCDBG(PPCDBG_SIGNAL, "setup32 - after copy to user 3 \n");
  ...
  regs->link = (unsigned long) frame->tramp;
   PPCDBG(PPCDBG_SIGNAL, "setup32 - before return trap address =  %lx \n", frame->tramp);
  return;
...
long sys32_rt_sigreturn(struct pt_regs * regs)
{
  ...
  PPCDBG(PPCDBG_SIGNAL, "sys32_rt_sigreturn - copied from user - signal_set=%lx \n", *(u64*)&signal_set);
...
asmlinkage long sys32_rt_sigaction(int sig, const struct sigaction32 *act, struct sigaction32 *oact, size_t sigsetsize)
{
  ...
  PPCDBG(PPCDBG_SIGNAL, "sys32_rt_sigaction - entered - sig=%x \n", sig);
    ...
    if (act)
       PPCDBG(PPCDBG_SYS32NI, " sys32_rt_sigaction flags = %lx \n" ,act->sa_flags);
	...
	if (act) {
        ...
        PPCDBG(PPCDBG_SIGNAL, " sys32_rt_sigaction flags(kernel) = %lx \n" ,new_ka.sa.sa_flags);
...
asmlinkage long sys32_rt_sigprocmask(u32 how, sigset32_t *set, sigset32_t *oset, size_t sigsetsize)
{
  ...
  PPCDBG(PPCDBG_SIGNAL, "sys32_rt_sigprocmask - entered how=%x \n", (int)how);
    ...
    PPCDBG(PPCDBG_SIGNAL, "sys32_rt_sigprocmask - *set=%lx \n", *(u64*)&s32);
  ...
  {
    PPCDBG(PPCDBG_SIGNAL, " sys32_rt_sigprocmask mask set = NULL \n" );
  }
	set_fs (KERNEL_DS);
...
int sys32_rt_sigsuspend(sigset32_t* unewset, size_t sigsetsize, int p3, int p4, int p6, int p7, struct pt_regs *regs)
{
  ...
  PPCDBG(PPCDBG_SYS64X, "sys32_rt_sigsuspend - running - pid=%ld current=%lx comm=%s \n", current->pid, current, current->comm);
  PPCDBG(PPCDBG_SIGNAL, "sys32_rt_sigsuspend - current block mask = %lx \n",current->blocked); 
...

{
    ...
    PPCDBG(PPCDBG_SIGNAL," sys32 alt stack entered sp = %lx \n", sp);
...
int do_signal32(sigset_t *oldset, struct pt_regs *regs)
{
  ...
  if (ka->sa.sa_flags & SA_SIGINFO) 
  {
    PPCDBG(PPCDBG_SIGNAL, " do sig - invoke setup rt signal newsp = %lx frame = %lx \n",
           newsp, frame);
    setup_rt_frame32(regs, (struct sigregs32*)(u64)frame, newsp);
  }
  else 
  {
   PPCDBG(PPCDBG_SIGNAL, " do sig - invoke setup non rt signal newsp = %lx frame = %lx \n",
            newsp, frame);
