...
prom_initialize_naca(unsigned long mem)
{
	...
	_naca->io_subsystem = IOS_INVALID;
        for (node = 0; prom_next_node(&node); ) {
                type[0] = 0;
...
#ifdef DEBUG_PROM
	prom_print(RELOC("    2) spinloop       = 0x"));
	prom_print_hex(spinloop);
	prom_print_nl();
	prom_print(RELOC("    2) *spinloop      = 0x"));
	prom_print_hex(*spinloop);
	prom_print_nl();
	prom_print(RELOC("    2) acknowledge    = 0x"));
	prom_print_hex(acknowledge);
	prom_print_nl();
	prom_print(RELOC("    2) *acknowledge   = 0x"));
	prom_print_hex(*acknowledge);
	prom_print_nl();
	prom_print(RELOC("    2) secondary_hold = 0x"));
	prom_print_hex(secondary_hold);
	prom_print_nl();
#endif
	/* look for cpus */
...
#endif
#ifdef DEBUG_YABOOT
	call_yaboot(yaboot->printf, RELOC("Location: 0x11a\n"));
#endif
