...
asmlinkage long sys32_fcntl(unsigned int fd, unsigned int cmd, unsigned long arg)
{
	...
	PPCDBG(PPCDBG_SYS32NI, "sys32_fcntl - running - pid=%ld, comm=%s \n", current->pid, current->comm);
...
asmlinkage long sys32_getrlimit(unsigned int resource, struct rlimit32 *rlim)
{
	...
	PPCDBG(PPCDBG_SYS32, "sys32_getrlimit - entered - pid=%ld, comm=%s\n", current->pid, current->comm);
	...
	PPCDBG(PPCDBG_SYS32, "sys32_getrlimit - exited w/ %x - resource=%x, rlim_max=%lx, rlim_cur=%lx - pid=%ld, comm=%s\n", 
               ret, resource, RESOURCE32(r.rlim_max), RESOURCE32(r.rlim_cur), current->pid, current->comm);
	return ret;
}
...
struct semid64_ds32 {
	struct ipc64_perm sem_perm;		  /* this structure is the same on ppc32 and ppc64 */
	unsigned int	  __pad1;
...
/*
 * sys32_ipc
 () is the de-multiplexer for the SysV IPC calls in 32bit emulation..
 *
...
static long do_sys32_semctl(int first, int second, int third, void *uptr)
{
 ...
 case SEM_STAT:
     if (third & IPC_64)
     {
       ...
       err = sys_semctl (first, second, third, fourth);
       set_fs (old_fs);
       err2 = copy_to_user (&usp->sem_perm, &s.sem_perm, sizeof(struct ipc64_perm) + 2*sizeof(time_t));
...
do_sys32_msgctl (int first, int second, void *uptr)
{
	...
	case MSG_STAT:
	    if (second & IPC_64)
		{
	      ...
	      err = sys_msgctl (first, second, (struct msqid_ds *)&m);
	      set_fs (old_fs);
          err2 = copy_to_user(&up->msg_perm, &m.msg_perm, sizeof(struct ipc64_perm) + 3*sizeof(time_t));
...
do_sys32_shmctl (int first, int second, void *uptr)
{
	...
	case SHM_STAT:
		if (second & IPC_64)
		{
		  ...
		  err2 = copy_to_user (&up->shm_perm, &s.shm_perm, sizeof(struct ipc64_perm) + 3*sizeof(time_t));
...
/* Note: it is necessary to treat first_parm, second_parm, and third_parm as unsigned ints, 
 * with the corresponding cast to a signed int to insure that the 
 * proper conversion (sign extension) between the register representation of a signed int (msr in 32-bit mode)
 * and the register representation of a signed int (msr in 64-bit mode) is performed.
 */
asmlinkage long sys32_ipc(u32 call, u32 first_parm, u32 second_parm, u32 third_parm, u32 ptr, u32 fifth)
{
  ...
  int third  = (int)third_parm;
	...
	PPCDBG(PPCDBG_SYS32, "sys32_ipc - entered - pid=%ld current=%lx comm=%s\n", current->pid, current, current->comm);
	...
	PPCDBG(PPCDBG_SYS32, "sys32_ipc - exited - pid=%ld current=%lx comm=%s\n", current->pid, current, current->comm);
...
asmlinkage long sys32_sendmsg(int fd, struct msghdr32* user_msg, unsigned int user_flags)
{
	...
	PPCDBG(PPCDBG_SYS32, "sys32_sendmsg - entered - pid=%ld, comm=%s \n", current->pid, current->comm);
...
out:
	...
	PPCDBG(PPCDBG_SYS32, "sys32_sendmsg - exited - pid=%ld, comm=%s \n", current->pid, current->comm);
	return err;
}
...
asmlinkage long sys32_recvmsg(int fd, struct msghdr32* user_msg, unsigned int user_flags)
{
	...
	PPCDBG(PPCDBG_SYS32, "sys32_recvmsg - entered - pid=%ld, comm=%s\n", current->pid, current->comm);
...
out:
	...
	PPCDBG(PPCDBG_SYS32, "sys32_recvmsg - exited - pid=%ld, comm=%s\n", current->pid, current->comm);
	return len;
}
...
/* ld.so expects the auxiliary table to start on a 16-byte boundary,
 * so this routine determines if the aux table in the stack meets this
 * criteria and if not rearranges it on the stack so it does start on a 
 * 16-byte boundary.
 */
static inline void shove_aux_table32(unsigned long sp)
{
	int argc;
	u_int p = 0;
  ...
  /* Get the argc value. */
  if (__get_user(argc, (u_int*)sp))
  {
		return;
  }

  /* Skip over the arguments. */
  sp += sizeof(int) + (argc + 1) * sizeof(u_int);

	/* skip over the environment pointers */
	do
  	{
		/* Get the value of the next 32-bit environment pointer.  */
    		if (__get_user(p, (u_int*)sp))
    			{
			return;
    			}
    /* Advance the stack pointer to the address of the next 32-bit pointer. */
		sp += sizeof(u_int);
	} while (p != 0);
  ...
  aux_start = sp;
	...
	do
  {
    /* Get the value of the next aux table entry. */
    if (__get_user(e, (u_int*)sp))
    {
      return;
    }
    /* Advance the stack pointer to the address of the next aux table entry (from 32-bit pgm). */
		sp += 2 * sizeof(u_int);
	} while (e != AT_NULL);

  /* Determine how far we have to move the aux table to get it onto a 16-byte boundary. */
	offset = ((aux_start + 15) & ~15) - aux_start;

  /* Move the aux table to the new 16-byte boundary area (if necessary). */
	if (offset != 0)
  {
		do
    {
			sp -= sizeof(u_int);
      /* Read the entry into kernel stg then Write the entry back into the stack at the new location. */
      if (__get_user(e, (u_int*)sp) || __put_user(e, (u_int*)(sp + offset)))
      { /* either the get or put failed. */
        return;
      }
		} while (sp > aux_start);
	}
...
asmlinkage int ppc64_newuname(struct new_utsname * name)
{
	...
	if (current->personality == PER_LINUX32 && !ret) {
		ret = copy_to_user(name->machine, "ppc\0\0", 8);
	}
	return ret;
}
