...
unsigned int hostfs_poll(struct file *file, struct poll_table_struct *table)
{
	not_implemented();
	return(-EINVAL);
}

int hostfs_ioctl(struct inode *ino, struct file *file, unsigned int code,
		 unsigned long data)
{
	not_implemented();
	return(-EINVAL);
...
int hostfs_fasync(int fd, struct file *file, int on)
{
	not_implemented();
	return(-EINVAL);
}
