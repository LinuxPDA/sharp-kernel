...
#include <asm/brecis/brecis_uart.h>
 ...
 */
static t_uart_brecis_regmap *kgdb_uart = (void *)(ATLAS_SAA9730_REG + SAA9730_UART_REGS_ADDR);
...
void brecis_kgdb_hook(void) 
{
	...
	OUTB(BRECIS_LCR_DATA8, &kgdb_uart->Lcr);
	...
	/* Set RTS/DTR active */
	OUTB(BRECIS_MCR_DTR | SAA9730_MCR_RTS, &kgdb_uart->Mcr);
...
int brecis_putDebugChar(char c)
{
        ...
        while (!(INB(&kgdb_uart->Lsr) & BRECIS_LSR_THRE))
                ;
