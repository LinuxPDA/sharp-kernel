...
//*********************** ...MaTed---
//#define ENABLE_MONITOR_DEBUG // ...MaTed--- enable debug kernel via monitor GDB
//*********************** ...MaTed---
/*
...
#ifndef NO_MM //...MaTed--- no tlb exceptions
...
#ifdef NO_MM //...MaTed--- No TLB so no exceptions
void panic_handle_mod( void )
{
  panic( "TLB modification attempted without TLB code" );
}
void panic_handle_tlbl( void )
{
  printk( KERN_EMERG "TLB exception address=[0x%08lX], EPC=[0x%08lX]",
	  read_32bit_cp0_register(CP0_BADVADDR),
	  read_32bit_cp0_register(CP0_EPC));
  panic( "TLB exception (load or instruction fetch) without TLB code");
}
void panic_handle_tlbs( void )
{
  printk( KERN_EMERG "TLB exception address =[0x%lx]",
	  read_32bit_cp0_register(CP0_BADVADDR));
  panic( "TLB exception (store) without TLB code" );
}
#endif
...
void __init trap_init(void)
{
	...
	return;		// Leave all vectors to the monitor
#endif   // ENABLE_MONITOR_DEBUG

//
//	...MaTed--- there is a bug that needs quashing
//	can't just use lower half -> someone didn't finish 
//	implementing this -- I turn this off <- needed for 
//	the ones with an EISA_bus though  ...MaTed---
#if 0
...
#else
	(void)set_except_vector(1, panic_handle_mod);
	(void)set_except_vector(2, panic_handle_tlbl);
	(void)set_except_vector(3, panic_handle_tlbs);
#endif // NO_MM
...
#ifndef NO_MM   // ...MaTed--- following not required - NO_MM
