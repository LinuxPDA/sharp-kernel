...
static int Test_argc = 4;
static char *Test_argv[]=
{
    "go",
#if 0	// fixed IP addresses - kendin switch is 16.2 ...MaTed---
    "ip=dhcp",
#else
    //    "ip=192.168.15.2",
#if 0	// reverse IP addresses - kendin 15.2
    "ip=192.168.16.2::192.168.15.2::Brecis:eth0:none",
    "ip=192.168.15.2::192.168.15.1:::eth1:none",
#else
    "ip=192.168.15.2::192.168.15.1::Brecis:eth0:none",
    "ip=192.168.16.2::192.168.15.2:::eth1:none",
#endif
    //    "ip=192.168.15.2:192.168.16.2",
#endif
    "root=/dev/nfs",
    "nfsroot=192.168.15.1:/export/mipsel"
};
static t_env_var Test_envp[]=
{
    {"memsize",		"0x02000000"},
    {"modetty0", 	"57600,n,8,1,hw"},
    {"bootserport", 	"tty0"},
    {"prompt",		"PMON"},
    {"cpuconfig",	""},
#if 0	// reverse MAC addresses
    {"ethaddr0",	"00.02.d8.00.00.54"},
    {"ethaddr1",	"00.02.d8.00.00.53"},
    {"ethaddr2",	"00.02.d8.00.00.55"},
#else
    {"ethaddr0",	"00.02.d8.00.00.53"},
    {"ethaddr1",	"00.02.d8.00.00.54"},
    {"ethaddr2",	"00.02.d8.00.00.55"},
#endif
    {"baseboardserial",	"0000000086"},
#if 0	// reverse
    {"ipaddr",		"192.168.16.2"},
...
static int *Test_prom_vec = 0L;
    ...
    argc	= Test_argc;
    argv	= Test_argv;
    envp	= (char **) &Test_envp[0];
    prom_vec	= Test_prom_vec;
#else
	unsigned int s;
#endif // MATEDSETUP
    ...
    for (i=0; i<argc; i++)
    {
        prom_printf ("*argv[%d]=>[0x%08x]=>[%s]\n", i, argv[i], argv[i]);
    }

    prom_printf ("envp=[0x%8x]\n", envp);
 
    i = 0;
    while ((((t_env_var *) envp)[i]).name != NULL)
    {
        prom_printf ("*envp[%d]=[%s]=[%s]\n", i,
	    (((t_env_var*) envp)[i]).name,
            (((t_env_var *) envp)[i]).val);
        i++;
    }
    prom_printf ("prom_vec=[0x%8x] == [0x%x]\n", prom_vec, prom_vec);
#endif // MATEDSETUP
