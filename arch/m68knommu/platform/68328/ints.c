...
void M68328_init_IRQ(void)
{
	...
	*(unsigned volatile long *)0xfffff304 = 0xffffffff;
}
 ...
 */
inline int M68328_do_irq(int vec, struct pt_regs *fp)
{
	int irq;
	int mask;
...
int M68328_get_irq_list(char *buf)
{
	int i, len = 0;
	irq_node_t *node;

	len += sprintf(buf+len, "Internal 68328 interrupts\n");
