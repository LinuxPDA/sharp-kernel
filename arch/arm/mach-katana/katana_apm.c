/*
 * linux/arch/arm/mach-katana/katan_apm.c
 *
...
void print_debug(char* argv)
{
	int i, len;
	char *p = argv;
	volatile unsigned long *ir03r = 0xd800400c;
	volatile unsigned long *ir04r = 0xd8004010;
...
static int katana_suspend_request;	/* 1 : do_suspend / 0:  */
...
void cpu_katana_idle(void);

static unsigned long mq9000_get_cmos_time(void)
{
...
console_verbose();
printk("powerirq_handler\n");
	if (irq == IRQ_GPIO_PWDN) {	/* suspend */
		if(pMQRegs->gp.gp0B.GpioData & 0x00000001) {
			/* release */
			count = 0;
		} else {
			/* Push */
			if (count++  > 150 && !katana_suspend_request) {
				count = 0;
				katana_suspend_request = 1;
			}
		}
	}
}
...
volatile MQ9000REGS *pMQRegs = (MQ9000REGS *)MQ9000_REGS_VBASE;
	...
	//set_GPIO_IRQ_edge(IRQ_TO_Timer_GPIO(IRQ_GPIO_PWDN), GPIO_FALLING_EDGE);
	set_GPIO_IRQ_edge(IRQ_TO_Timer_GPIO(IRQ_GPIO_PWDN), GPIO_RISING_EDGE);
	//pMQRegs->gp.gp04.GpioEnable |= 0x00000001;
	pMQRegs->gp.gp03.GpioWakeEn = 0x00010000;

	standbys_pending = 0;
	suspends_pending = 0;
	katana_suspend_request = 0;

	if (request_irq(IRQ_GPIO_PWDN,
			powerirq_handler, SA_INTERRUPT, "apm", powerirq_handler)) {
		printk(KERN_NOTICE "apm: Couldn't get irq %d.\n", IRQ_GPIO_PWDN);
		return -ENODEV;
	}
printk("GP00R=0x%08x\n", pMQRegs->gp.gp00.TimerCtrl);
printk("GP01R=0x%08x\n", pMQRegs->gp.gp01.GpioDir);
printk("GP03R=0x%08x\n", pMQRegs->gp.gp03.GpioWakeEn);
printk("GP04R=0x%08x\n", pMQRegs->gp.gp04.GpioEnable);
printk("GP0DR=0x%08x\n", pMQRegs->gp.gp0D.TimerLEDInt1);
printk("GP0ER=0x%08x\n", pMQRegs->gp.gp0E.TimerLEDInt2);
printk("GP1AR=0x%08x\n", pMQRegs->gp.gp1A.GpioModeEnable);
...
static void __exit apm_exit(void)
{
	...
	free_irq(IRQ_GPIO_PWDN, powerirq_handler);
