...
int check_flash(void)
{
  ...
  if ( strncmp((char *)(FLASH_MEM_BASE_INF+STRING_ID),"SHARP SL-series ",16)==0 )
     return 1;
  else
     return 0;
}
...
int check_sum(void)
{
  ...
  if ( !check ) {
    ...
    checksum = (unsigned short)crc;
    printk("check sum = %04x\n",checksum);
    check = 1;
  }
  return check;
}
...
{
  if ( check_flash() ) {
    switch (FLASH_DATA_SHORT(FLASH_MEM_BASE_INF+MODEL)) {
    case SL5000D:
      return "SL-5000D";
    case SL5500:
      return "SL-5500";
...
{
  unsigned int romver = FLASH_DATA_INT(FLASH_MEM_BASE_INF+VERSION);
  char *sumstr = (char*)FLASH_MEM_BASE_SUM;
#if 0
  unsigned short cksum = strtol(sumstr,sumstr+4,16);
#else
  char buf[5];
...
{
  if ( check_flash() ) {
    switch (FLASH_DATA_SHORT(FLASH_MEM_BASE_INF+COUNTRY)) {
      case USA:
	return "US";
      case GER:
	return "DE";
