/* $Id: setup.c,v 1.2 2001/04/11 01:30:54 randy Exp $
 *
...
//FIXME	memory_start = (unsigned long)PAGE_OFFSET+__MEMORY_START;
	/* Default is 4Mbyte. */
//FIXME	memory_end = (unsigned long)PAGE_OFFSET+0x00400000+__MEMORY_START;
...
void __init setup_arch(char **cmdline_p)
{
	...
	code_resource.start = (unsigned long)virt_to_bus(&_text);
	code_resource.end = (unsigned long)virt_to_bus(&_etext)-1;
	data_resource.start = (unsigned long)virt_to_bus(&_etext);
	data_resource.end = (unsigned long)virt_to_bus(&_edata)-1;
...
#define PFN_PHYS(x)	((x) << PAGE_SHIFT)
	...
	bootmap_size = 0;
/* FIXME    bootmap_size = init_bootmem_node(NODE_DATA(0), start_pfn,
...
*/
	 ...
	 */
	{
		unsigned long curr_pfn=0, last_pfn, pages;

		/*
		 * We are rounding up the start address of usable memory:
		 */
//FIXME		curr_pfn = PFN_UP(__MEMORY_START);
		/*
	 ...
	 * an invalid RAM area.
	 */
/* FIXME	reserve_bootmem(__MEMORY_START+PAGE_SIZE, (PFN_PHYS(start_pfn) + 
...
*/
	 ...
	 */
//FIXME	reserve_bootmem(__MEMORY_START, PAGE_SIZE);
