...
static void page_detail(struct page *page)
{
	...
	printk("%p: %2d %08lx %p [%c%c%c%c] %c%c [%c%c%c]\n",
			page_address(page),
			page_count(page),
			page->age,
			page->mapping,
			...
			PageActive(page) ? 'a' : '-',
			PageInactiveClean(page) ? 'c' : '-',
			PageInactiveDirty(page) ? 'd' : '-');
}

/*
 * This version collects statistics
 */
static int anon_pages, slab_pages, resvd_pages, unused_pages, ramdisk_pages;
...
static void page_statistics(struct page *page)
{
	...
	printk("  anon: %d, slab: %d, reserved: %d, free: %d, ramdisk: %d\n",
		anon_pages, slab_pages, resvd_pages, unused_pages, ramdisk_pages);
}
...
static void show_zone_info(zone_t *zone, void (*fn)(struct page *))
{
	...
	printk("  total %ld, free %ld, inactive clean %ld, inactive dirty %ld\n",
		zone->size,
		zone->free_pages,
