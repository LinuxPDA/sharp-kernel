 ...
 * short or int arrays we might have an alignment problem.  Declaring these
 * as taking ptrs to unsigned long should at least generate warnings.
 *
 * Apparent original author: 2000/02/09 03:28:31 davem
 *
...
static __inline__ int ext2_ffz(unsigned int x)
{
  ...
  /* Calculate the bit position of the least signficant '1' bit in x
   * (since x has been changed this will actually be the least signficant '0' bit in the original x).
   * Note: (x & -x) gives us a mask that is the LEAST significant (RIGHT-most) 1-bit of the value in x.
   */
