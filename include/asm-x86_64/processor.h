...
#define INIT_THREAD  {						\
	debugreg: { [0 ... 7] = 0 },	/* debugging registers */	\
}
...
#define STACKDESC rsp2: stackfault_stack,
...
/* DF currently shares the same stack on all CPUs. Hopefully only one gets into this unfortunate
   condition at a time. Cannot do the same for SF because that can be easily triggered by user space. */ 
#define INIT_TSS  {						\
	rsp1: doublefault_stack, 		\
	STACKDESC \
}
...
/* We probably don't want to support microcode uploads, do we? If not,
   we can kill this --pavel */
struct microcode {
	unsigned int hdrver;
	unsigned int rev;
	unsigned int date;
...
// use the SSE2 prefetch. could as well use the 3dnow one too.
