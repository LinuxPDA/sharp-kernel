...
#define check_switch_to(prev,next,last) do {					\
	unsigned long myrsp;			\
	if (&myrsp < 0xffff000000000000)			\
		panic("switch_to: prev (%lx) running off userland stack (%lx)\n", prev, &myrsp);	\
	if (next->thread.rsp < 0xffff000000000000)			\
		panic("switch_to: next (%lx) running off userland stack (%lx)\n", next, next->thread.rsp);\
	} while(0)
#else
#define check_switch_to(prev,next,last) do { } while(0)
#endif

#define switch_to(prev,next,last) do {					\
	check_switch_to(prev,next,last);				\
				      ...
				      unsigned long new, int size)
{
	...
	case 2:
		...
		return prev;
	case 4:
		__asm__ __volatile__(LOCK_PREFIX "cmpxchgl %k1,%k2"
				     : "=a"(prev)
				     : "q"(new), "m"(*__xg(ptr)), "0"(old)
				     : "memory");
		return prev;
...
#define likely(x)   __builtin_expect((x)!=0, !0)
