 ...
 *	06-Jul-2004 Lineo Solutions, Inc.  for PXA27x
 */

#if defined(CONFIG_CPU_PXA27X)
#define PXA_IRQ_SKIP	0	/* The first 8 IRQs are reserved */
#else
...
#if defined(CONFIG_CPU_PXA27X)
#define	IRQ_SSP3	PXA_IRQ(0)	/* SSP_3 Service Request */
#define	IRQ_MSL		PXA_IRQ(1)	/* MSL Interface interrupt */
#define	IRQ_USBH2	PXA_IRQ(2)	/* USB Host 2 */
#define	IRQ_USBH1	PXA_IRQ(3)	/* USB Host 1 */
#define	IRQ_KEYPAD	PXA_IRQ(4)	/* Keypad controller interrupt */
#define	IRQ_MEM_STK	PXA_IRQ(5)	/* Memory stick interrupt */
#define	IRQ_PWR_I2C	PXA_IRQ(6)	/* Power I2C interrupt */
#define	IRQ_OST_4_11	PXA_IRQ(7)	/* OS timer matches registers 4-11 */
#endif
...
#if defined(CONFIG_CPU_PXA27X)
#define	IRQ_USIM	PXA_IRQ(15)	/* Smart card interface status/error */
#define	IRQ_SSP2	PXA_IRQ(16)	/* SSP_2 service request */
#endif
...
#undef NR_IRQS
#define	NR_IRQS		(IRQ_GPIO(GPIO_MAX) + 1)

#if defined(CONFIG_SA1111)

#define IRQ_SA1111_START	(IRQ_GPIO(GPIO_MAX) + 1)
...
#else
#define LUBBOCK_IRQ(x)	(IRQ_GPIO(GPIO_MAX) + 1 + (x))
#endif
...
#define IRQ_ASIC3_A		IRQ_GPIO(GPIO_MAX)
...
#define TC6393_IRQ(x)      (IRQ_GPIO(GPIO_MAX) + 1 + (x))
...
#else
#define LCM_IRQ(x)	(IRQ_GPIO(GPIO_MAX) + 1 + (x))
#endif
