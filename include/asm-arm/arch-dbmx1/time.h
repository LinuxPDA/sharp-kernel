 ...
 *
 *  linux/include/asm-arm/arch-db1mx1/time.h
 *
 * Based on:
 ...
 */
#define TIMER0_VA_BASE (IO_ADDRESS(DBMX1_TIM1_BASE)+0x00000000)
#define TIMER1_VA_BASE (IO_ADDRESS(DBMX1_TIM2_BASE)+0x00000000)
 ...
 */
static void
dbmx1_timer_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
...
#ifdef CONFIG_RTHAL
static void tim1_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
}

static struct irqaction irq59  = { tim1_interrupt, SA_INTERRUPT, 0, "tim1", NULL, NULL};
#endif
...
setup_timer(void)
{
	...
	timer_irq.handler = dbmx1_timer_interrupt;
	...
	timer1->TimerCompare = LATCH;
	timer1->TimerControl = (TIM_PERCLK1 | TIM_INTEN | TIM_ENAB);

	/*
	 * Make irqs happen for the system timer
	 */
	setup_arm_irq(IRQ_TIMER2_INT, &timer_irq);
	gettimeoffset = dbmx1_gettimeoffset;
#ifdef CONFIG_RTHAL
	setup_arm_irq(IRQ_TIMER1_INT, &irq59);
#endif
}
