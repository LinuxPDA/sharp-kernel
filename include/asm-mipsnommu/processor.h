...
#ifndef NO_MM	//...MaTed--- variable init_mmap is not used - don't need initialization
...
// ...MaTed--- test for GDB kernel crash, double size of stack
 ...
 * 
 * Change regs->cp0_status to not set ST0_KSU and KU_USER.  See original line
 * here:
 *    regs->cp0_status = (regs->cp0_status & ~(ST0_CU0|ST0_KSU)) | KU_USER;\
 * 
 * This is because the uClinux (for MIPS) is configured not to use separate
 * user and kernel/supervisor modes.
 */
#define start_thread(regs, new_pc, new_sp) do {				\
	/* New thread looses kernel privileges. */			\
	regs->cp0_status = (regs->cp0_status & ~(ST0_CU0));             \
	regs->cp0_epc = new_pc;						\
	regs->regs[29] = new_sp;					\
