...
#else
#define USER_DS		((mm_segment_t) { (unsigned long) 0L })
#endif
...
/*
 * In uClinux we need to change this because the __access_ok macro 
 * doesn't work properly.  In regular Linux which assumes that an MMU is 
 * attached to the core, the TLB moves all userspace applications to below the
 * 16GB mark, and the kernel code above it.  Thus in many places checks are
 * made to ensure that the userspace application remains below it.  This isn't
 * possible in an environment that doesn't have an MMU.  We now need to change
 * get_user and put_user to behave the same as __get_user and __put_user.
 *
 * **************************************************************************
 * This doesn't seem to work, if put_user is redefined.  get_user is required
 * to be redefined.
 * **************************************************************************
 *
 * Reverting all of the changes to back to normal to prevent any global 
 * problems, until it is properly corrected.
 *
 */
//#undef get_user
//#undef put_user
