...
SYS(sys_setgroups, 2)
     /*  SYS(sys_ni_syscall, 0) was old_select now again select. See comments in include/asm-mipsnommu/unistd.h for this same syscall. */ /* old_select */
SYS(sys_select, 5)
SYS(sys_symlink, 2)
SYS(sys_lstat, 2)
...
SYS(sys_reboot, 3)
SYS(old_readdir, 3)
     /* SYS(old_mmap, 6) was old_mmap but include/asm-mipsnommu/unistd.h for this syscall.  Should be sys_mmap.	back to old_mmap as traps.c points to old and not sys_mmap. */	/* 4090 */
SYS(old_mmap, 6)                                /* 4090 */
SYS(sys_munmap, 2)
SYS(sys_truncate, 2)
SYS(sys_ftruncate, 2)
...
SYS(sys_vfork, 0) /* Added for uClinux.  */
