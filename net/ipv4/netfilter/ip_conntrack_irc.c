/* IRC extension for IP connection tracking, Version 1.14
 * (C) 2000 by Harald Welte <laforge@gnumonks.org>
 * based on RR's ip_conntrack_ftp.c	
 *
 * ip_conntrack_irc.c,v 1.14 2000/11/07 18:26:42 laforge Exp
 *
		...
		struct ip_conntrack *ct, enum ip_conntrack_info ctinfo)
{
	...
	if (tcp_v4_check(tcph, tcplen, iph->saddr, iph->daddr,
		...
		DEBUGP
		    ("irc_help: bad csum: %p %u %u.%u.%u.%u %u.%u.%u.%u\n",
	...
	while (data < (data_limit - (22 + MAXMATCHLEN))) {
		...
		DEBUGP
		    ("ip_conntrack_irc_help: DCC found in connection %u.%u.%u.%u:%u %u.%u.%u.%u:%u\n",
		     NIPQUAD(iph->saddr), tcph->source,
		     NIPQUAD(iph->daddr), tcph->dest);
		for (i = 0; i < NUM_DCCPROTO; i++) {
			if (memcmp
			    (data, dccprotos[i].match,
			     dccprotos[i].matchlen) == 0) {
				DEBUGP
				    ("ip_conntrack_irc_help: DCC %s detected\n",
				     dccprotos[i].match);
				data += dccprotos[i].matchlen;
				if (!parse_dcc
				    ((char *) data, data_limit, &dcc_ip,
				     &dcc_port, &addr_beg_p, &addr_end_p)) {
					DEBUGP
					    ("ip_conntrack_irc_help: DCC request with bound addr %u.%u.%u.%u:%u found\n",
					     HIPQUAD(dcc_ip),
					     dcc_port);
					...
					info->port = dcc_port;
					DEBUGP
					    ("ip_conntrack_irc_help: wrote info: seq=%u (ofs=%u), len=%d\n",
					     info->seq,
					     (addr_end_p - _data),
					     info->len);
					...
					DEBUGP
					    ("ip_conntrack_irc_help: expect_related %u.%u.%u.%u:%u - %u.%u.%u.%u:%u\n",
					     NIPQUAD(t.src.ip),
...
static int __init init(void)
{
	...
	for (i = 0; (i < MAX_PORTS) && ports[i]; i++) {
		...
		if (ret) {
			printk("ip_ct_irc: ERROR registering port %d\n",
				ports[i]);
			fini();
			return 1;
		}
...
static void fini(void)
{
	int i;
	for (i = 0; (i < MAX_PORTS) && ports[i]; i++) {
		DEBUGP("ip_conntrack_irc: unregistering port %d\n",
		       ports[i]);
