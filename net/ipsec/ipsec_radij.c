 ...
 * for more details.
 *
 * RCSID $Id: ipsec_radij.c,v 1.48 2001/06/15 04:12:56 rgb Exp $
 */
...
int
ipsec_makeroute(struct sockaddr_encap *eaddr, struct sockaddr_encap *emask, struct sa_id said, uint32_t pid)
{
	struct eroute *retrt;
	int error = 0;
	char sa[SATOA_BUF];
	...
	if (debug_eroute) {
		...
		KLIPS_PRINT(debug_eroute, 
			    ...
			    "attempting to insert eroute for %s->%s, SA: %s, PID:%d\n",
			    buf1,
			    buf2,
			    sa_len ? sa : " (error)",
			    pid);
