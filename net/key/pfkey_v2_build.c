/* $USAGI: pfkey_v2_build.c,v 1.1 2001/12/23 16:01:12 mk Exp $ */
...
char pfkey_v2_build_c_version[] = "$Id: pfkey_v2_build.c,v 1.1.1.1 2001/05/22 06:14:05 miyazawa Exp $";
...
#ifdef CONFIG_IPSEC_PFKEY_DEBUG
# define DEBUGGING(args...) \
         pr_debug("klips_debug:" args)
#else
# define DEBUGGING(args...)
#endif
		    ...
		    struct sockaddr*	address)
{
	...
	case AF_INET:
		...
		in_ntop(&((struct sockaddr_in*)address)->sin_addr, ipaddr_txt);
		break;
	case AF_INET6:
		DEBUGGING(
			"pfkey_address_build: found address family AF_INET6.\n");
		saddr_len = sizeof(struct sockaddr_in6);
		in6_ntop(&((struct sockaddr_in6*)address)->sin6_addr, ipaddr_txt);
		break;
	default:
		DEBUGGING(
