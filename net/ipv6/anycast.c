/* $USAGI: anycast.c,v 1.1 2001/12/23 15:02:41 yoshfuji Exp $ */
...
/* XXX ip6_onlink() really belongs in net/core.c */

static int ip6_onlink(struct in6_addr *addr, struct net_device *dev)
{
...
int inet6_ac_check(struct sock *sk, struct in6_addr *addr, int ifindex)
{
	...
	for (pac = sk->net_pinfo.af_inet6.ipv6_ac_list; pac;
	     pac = pac->acl_next) {
	...
	for (dev = dev_base; dev; dev = dev->next) {
		...
		for (im = idev->ac_list; im; im = im->aca_next) {
			...
			len += sprintf(buffer + len, 
				       "%-4d %-15s ", dev->ifindex, dev->name);

			for (i = 0; i < 16; i++)
				len += sprintf(buffer + len, 
					       "%02x", im->aca_addr.s6_addr[i]);
