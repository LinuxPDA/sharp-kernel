/*======================================================================
    ...
    This code is based on drivers/pcmcia/sa1100_generic.c
    ...
    ChangeLog:
    04-30-2002 Richard Rau: For Discovery CF controler.
...
sa1100_pcmcia_socket[CPLD_PCMCIA_MAX_SOCK];
...
static int __init sa1100_pcmcia_driver_init(void){
  ...
  struct pcmcia_state state[CPLD_PCMCIA_MAX_SOCK];
  struct pcmcia_state_array state_array;
  unsigned int i, clock;
  unsigned long mecr;

  printk(KERN_INFO "Cotulla PCMCIA (CS release %s)\n", CS_RELEASE);
...
*/
  ...
  for(i=0; i<sa1100_pcmcia_socket_count; ++i){
    ...
    printk(KERN_ERR "virtual_io=0x%X, phy_attr=0x%X, phy_mem=0x%X\n", 
    	   sa1100_pcmcia_socket[i].virt_io, sa1100_pcmcia_socket[i].phys_attr,
    	   sa1100_pcmcia_socket[i].phys_mem);
  ...
  printk(KERN_ERR "sa1100: initialization complete\n");
...
//	printk("prev : %d , now: %d\n", prev_state->detect, state->detect);
	if(state->detect!=prev_state->detect)
	{
		DEBUG(2, "%s(): card detect value %u\n", __FUNCTION__, state->detect);
		printk("card detect chanege\n");
...
static void sa1100_pcmcia_task_handler(void *data)
{
  	...
  	do
  	{
		...
		if(irq_status>0)
		{
			...
			if((events = sa1100_pcmcia_events(&state[i],
			...
			sa1100_pcmcia_socket[i].handler(sa1100_pcmcia_socket[i].handler_info, events);
		}
#if defined(CONFIG_SABINAL_DISCOVERY)
		if (vcc_on && (ASIC3_GPIO_PSTS_D & CF_DETECT)) {
			events = 0;
			events|=sa1100_pcmcia_socket[0].cs_state.csc_mask&SS_DETECT;
			sa1100_pcmcia_socket[0].handler(sa1100_pcmcia_socket[0].handler_info, events);
		}
#endif
	} while(all_events);
...
static int sa1100_pcmcia_set_io_map(unsigned int sock,
  ...
  if(map->flags&MAP_ACTIVE){
    ...
    // CYB MECR_BSIO_SET(mecr, sock, sa1100_pcmcia_mecr_bs(speed, clock));

    sa1100_pcmcia_socket[sock].speed_io=speed;

    //printk(KERN_ERR "%s(): FAST%u %lx  BSM%u %lx  BSA%u %lx  BSIO%u %lx\n",
	...
	(map->flags&MAP_USE_WAIT)?"USE_WAIT ":"");
  	...
  	if (map->flags&MAP_ACTIVE) {
     		...
     		*/
            	...
            	//DEBUG(KERN_ERR "%s(): FAST%u %lx  BSM%u %lx  BSA%u %lx  BSIO%u %lx\n",
        	//  __FUNCTION__, sock, MECR_FAST_GET(mecr, sock), sock,
        	//  MECR_BSM_GET(mecr, sock), sock, MECR_BSA_GET(mecr, sock), 
        	//  sock, MECR_BSIO_GET(mecr, sock));
...
static int sa1100_pcmcia_proc_status(char *buf, char **start, off_t pos,
  ...
  unsigned int clock;// CYB = cpufreq_get(0);
  unsigned long mecr; // CYB  = MECR;
...
static void sa1100_pcmcia_update_mecr(unsigned int clock){
  ...
  for(sock = 0; sock < CPLD_PCMCIA_MAX_SOCK; ++sock){
