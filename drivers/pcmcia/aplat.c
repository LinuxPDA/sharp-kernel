...
#if defined(CONFIG_S1C38000_TTE301)

#define CONFIG_ISA
#define	PCMCIA_MAP_BASE		(IO_START_2 + 0x00000000)
#define	PCMCIA_IO_MAP_BASE	(PCMCIA_MAP_BASE + 0x00000000)
#define	PCMCIA_MEM_MAP_BASE	(PCMCIA_MAP_BASE + 0x00020000)
#define	PCMCIA_IO_BASE		(IO_BASE_2 + 0x00000000)
#define	PCMCIA_REG_BASE		(IO_BASE_2 + 0x00800000)
#define	IRQ_PCMCIA_DETECT	IRQ_GPIO_PCMCIA_DETECT
#define	IRQ_PCMCIA_CARD		IRQ_GPIO_PCMCIA_CARD
#define	DATA_PORT_OFFSET	2

#elif defined(CONFIG_DBMX1_TPM102)

#define CONFIG_ISA
#define	PCMCIA_MAP_BASE		APLAT_PCMCIA_MEM_BASE
#define	PCMCIA_IO_MAP_BASE	(PCMCIA_MAP_BASE + 0x00000000)
#define	PCMCIA_MEM_MAP_BASE	(PCMCIA_MAP_BASE + 0x00010000)
#define	PCMCIA_IO_BASE		IO2_ADDRESS(APLAT_PCMCIA_MEM_BASE)
#define	PCMCIA_REG_BASE		IO2_ADDRESS(APLAT_PCMCIA_REG_BASE)
#define	IRQ_PCMCIA_DETECT	IRQ_GPIO_PCMCIA_DETECT
#define	IRQ_PCMCIA_CARD		IRQ_GPIO_PCMCIA_CARD
...
/* Default base address for i82365sl and other ISA chips */
static int i365_base = PCMCIA_REG_BASE;
/* Should we probe at 0x3e2 for an extra ISA controller? */
static int extra_sockets = 0;
/* Specify a socket number to ignore */
static int ignore = -1;
...
static int irq_list[16] = { -1 };
/* The card status change interrupt -- 0 means autoselect */
static int cs_irq = 14;
#define I365_MASK		((1 << 15) | (1 << 14))
...
/* External clock time, in nanoseconds.  125 ns = 8 MHz */
static int cycle_time = 125;
...
static u_char i365_get(u_short sock, u_short reg)
{
	...
	outb(reg, port); val = inb(port + DATA_PORT_OFFSET);
...
static void i365_set(u_short sock, u_short reg, u_char data)
{
	...
	outb(val, port); outb(data, port + DATA_PORT_OFFSET);
	spin_unlock_irqrestore(&bus_lock,flags);
    }
}
...
void i365_set_int()
{
    i365_bflip(0, 0x2f, 0x02, 1);
    i365_bflip(0, 0x2f, 0x02, 0);
    i365_bflip(0, 0x2f, 0x02, 1);
}
...
static void __init add_pcic(int ns, int type)
{
	...
	t[i].cap.irq_mask = 0;
#ifdef CONFIG_ARCH_APLAT
	t[i].cap.io_offset = PCMCIA_IO_BASE;
	t[i].cap.pci_irq = IRQ_PCMCIA_CARD;
	t[i].cs_irq = cs_irq;
#else
	t[i].cs_irq = isa_irq;
#endif
...
static int i365_set_io_map(u_short sock, struct pccard_io_map *io)
{
    ...
    io->start = (io->start & 0x00ffffff) + PCMCIA_IO_MAP_BASE;
    io->stop = (io->stop & 0x00ffffff) + PCMCIA_IO_MAP_BASE;
    /* Turn off the window before changing anything */
    if (i365_get(sock, I365_ADDRWIN) & I365_ENA_IO(map))
	i365_bclr(sock, I365_ADDRWIN, I365_ENA_IO(map));
#if 1
    i365_set_pair(sock, I365_IO(map)+I365_W_START, io->start & 0xffff);
    i365_set_pair(sock, I365_IO(map)+I365_W_STOP, io->stop & 0xffff);
#else
    i365_set_pair(sock, I365_IO(map)+I365_W_START, 0);
    i365_set_pair(sock, I365_IO(map)+I365_W_STOP, 0x0fff);
#endif
...
static int i365_get_mem_map(u_short sock, struct pccard_mem_map *mem)
{
    ...
    mem->sys_start = ((u_long)(i & 0x0fff) << (12 + PCMCIA_ADDRESS_SHIFT))
	+ PCMCIA_MAP_BASE;
    ...
    mem->sys_stop = ((u_long)(i & 0x0fff) << (12 + PCMCIA_ADDRESS_SHIFT))
	+ 0x0fff + PCMCIA_MAP_BASE;
    
    i = i365_get_pair(sock, base+I365_W_OFF);
    mem->flags |= (i & I365_MEM_WRPROT) ? MAP_WRPROT : 0;
    mem->flags |= (i & I365_MEM_REG) ? MAP_ATTRIB : 0;
    mem->card_start = ((u_int)(i & 0x3fff) << 12) + ((mem->sys_start - PCMCIA_MAP_BASE) >> PCMCIA_ADDRESS_SHIFT);
...
static int i365_set_mem_map(u_short sock, struct pccard_mem_map *mem)
{
	...
	mem->sys_stop = mem->sys_start +
	    (socket[sock].cap.map_size << PCMCIA_ADDRESS_SHIFT) - 1;
    if (mem->sys_start < PCMCIA_MEM_MAP_BASE) {
	mem->sys_stop = mem->sys_stop - mem->sys_start + PCMCIA_MEM_MAP_BASE;
	mem->sys_start = PCMCIA_MEM_MAP_BASE;
    } else {
	mem->sys_start = (mem->sys_start & 0x00ffffff) + PCMCIA_MAP_BASE;
	mem->sys_stop = (mem->sys_stop & 0x00ffffff) + PCMCIA_MAP_BASE;
    }
    /* Turn off the window before changing anything */
    ...
    base = I365_MEM(map);
    i = ((mem->sys_start - PCMCIA_MAP_BASE) >> (12 + PCMCIA_ADDRESS_SHIFT))
	& 0x0fff;
    if (mem->flags & MAP_16BIT) i |= I365_MEM_16BIT;
    if (mem->flags & MAP_0WS) i |= I365_MEM_0WS;
    i365_set_pair(sock, base+I365_W_START, i);
    
    i = ((mem->sys_stop - PCMCIA_MAP_BASE) >> (12 + PCMCIA_ADDRESS_SHIFT))
	& 0x0fff;
    ...
    i = ((mem->card_start - ((mem->sys_start - PCMCIA_MAP_BASE) >> PCMCIA_ADDRESS_SHIFT)) >> 12) & 0x3fff;
...
static int pcic_init(unsigned int s)
{
	...
	mem.sys_stop = 0x1000;
	io.start = PCMCIA_IO_MAP_BASE;
	io.stop = PCMCIA_IO_MAP_BASE + (PAGE_SIZE << PCMCIA_ADDRESS_SHIFT) - 1;
	mem.sys_start = PCMCIA_MEM_MAP_BASE;
	mem.sys_stop = PCMCIA_MEM_MAP_BASE + 0x0fff;
...
static int __init init_i82365(void)
{
    ...
    request_irq(IRQ_PCMCIA_DETECT, pcic_interrupt, 0, "i82365", pcic_interrupt);
    ...
    DBMX1_GPIO_ICR1_A = (DBMX1_GPIO_ICR1_A & 0xff0fffff) | 0x00a00000;
#endif
