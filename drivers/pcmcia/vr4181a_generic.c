...
#define MAP_SIZE 0x00800000

/* This structure maintains housekeeping state for each socket, such
 * as the last known values of the card detect pins, or the Card Services
 * callback value associated with the socket:
 */
static struct vr4181a_pcmcia_socket *pcmcia_socket;
...
static int  vr4181a_pcmcia_driver_init(void);
static void vr4181a_pcmcia_driver_shutdown(void);
static void vr4181a_pcmcia_task_handler(void *data);
static void vr4181a_pcmcia_poll_event(unsigned long data);
static void vr4181a_pcmcia_interrupt(int irq, void *dev, struct pt_regs *regs);
static struct tq_struct vr4181a_pcmcia_task;

#ifdef CONFIG_PROC_FS
static int vr4181a_pcmcia_proc_status(char *buf, char **start, 
...
static int vr4181a_pcmcia_init(u32 sock);
static int vr4181a_pcmcia_suspend(u32 sock);
static int vr4181a_pcmcia_register_callback(u32 sock, 
		void (*handler)(void *, u32), void *info);
static int vr4181a_pcmcia_inquire_socket(u32 sock, socket_cap_t *cap);
static int vr4181a_pcmcia_get_status(u32 sock, u_int *value);
static int vr4181a_pcmcia_get_socket(u32 sock, socket_state_t *state);
static int vr4181a_pcmcia_set_socket(u32 sock, socket_state_t *state);
static int vr4181a_pcmcia_get_io_map(u32 sock, struct pccard_io_map *io);
static int vr4181a_pcmcia_set_io_map(u32 sock, struct pccard_io_map *io);
static int vr4181a_pcmcia_get_mem_map(u32 sock, struct pccard_mem_map *mem);
static int vr4181a_pcmcia_set_mem_map(u32 sock, struct pccard_mem_map *mem);
#ifdef CONFIG_PROC_FS
static void vr4181a_pcmcia_proc_setup(u32 sock, struct proc_dir_entry *base);
#endif

static struct pccard_operations vr4181a_pcmcia_operations = {
	vr4181a_pcmcia_init,
	vr4181a_pcmcia_suspend,
	vr4181a_pcmcia_register_callback,
	vr4181a_pcmcia_inquire_socket,
	vr4181a_pcmcia_get_status,
	vr4181a_pcmcia_get_socket,
	vr4181a_pcmcia_set_socket,
	vr4181a_pcmcia_get_io_map,
	vr4181a_pcmcia_set_io_map,
	vr4181a_pcmcia_get_mem_map,
	vr4181a_pcmcia_set_mem_map,
#ifdef CONFIG_PROC_FS
	vr4181a_pcmcia_proc_setup
#endif
};
...
static int __init vr4181a_pcmcia_driver_init(void)
{
	servinfo_t info;
	...
	pcmcia_low_level=&tcs8000_pcmcia_ops;
#else
#error  
#endif
	pcmcia_init.handler=vr4181a_pcmcia_interrupt;
	...
	pcmcia_socket = 
		kmalloc(sizeof(struct vr4181a_pcmcia_socket) * socket_count, 
				GFP_KERNEL);
	if (!pcmcia_socket) {
	...
	memset(pcmcia_socket, 0,
			sizeof(struct vr4181a_pcmcia_socket) * socket_count);
			
	/* 
	 * Assuming max of 2 sockets, which the Vr4181a supports.
	...
	for(i=0; i < socket_count; i++) {
		...
		if (i == 0) {
			pcmcia_socket[i].virt_io = 
				KSEG1ADDR(VR4181A_PCMCIA_IO_BASE_0);
			pcmcia_socket[i].phys_attr =
				(memaddr_t)VR4181A_PCMCIA_ATTR_BASE_0;
			pcmcia_socket[i].phys_mem =
				(memaddr_t)VR4181A_PCMCIA_MEM_BASE_0;
		}
		else  {
			pcmcia_socket[i].virt_io = 
				KSEG1ADDR(VR4181A_PCMCIA_IO_BASE_1);
			pcmcia_socket[i].phys_attr =
				(memaddr_t)VR4181A_PCMCIA_ATTR_BASE_1;
			pcmcia_socket[i].phys_mem =
				(memaddr_t)VR4181A_PCMCIA_MEM_BASE_1;
		}
	}

	/* Only advertise as many sockets as we can detect: */
	if(register_ss_entry(socket_count, &vr4181a_pcmcia_operations)<0){
	 ...
	 */
	vr4181a_pcmcia_poll_event(0);

	DEBUG(1, "vr4181a: initialization complete\n");
	return 0;

}  /* vr4181a_pcmcia_driver_init() */

module_init(vr4181a_pcmcia_driver_init);

static void __exit vr4181a_pcmcia_driver_shutdown(void)
{
	int i;

	del_timer_sync(&poll_timer);
	unregister_ss_entry(&vr4181a_pcmcia_operations);
	...
	DEBUG(1, "vr4181a: shutdown complete\n");
}

module_exit(vr4181a_pcmcia_driver_shutdown);

static int vr4181a_pcmcia_init(unsigned int sock) { return 0; }

static int vr4181a_pcmcia_suspend(unsigned int sock)
{
	return 0;
}
...
vr4181a_pcmcia_events(struct pcmcia_state *state, 
...
}  /* vr4181a_pcmcia_events() */


/* 
 * Vr4181a_pcmcia_task_handler()
...
#if 0 /*@@@@@*/
	if ( !tcs8000_ready () ) {
		printk (KERN_ERR "PCMCIA interrupt detected.\n" );
	}
#endif
	for(i=0; i<socket_count; i++)  {
		...
		events = vr4181a_pcmcia_events(&state, 
...
static struct tq_struct vr4181a_pcmcia_task = {
	routine: vr4181a_pcmcia_task_handler
};


static void vr4181a_pcmcia_poll_event(unsigned long dummy)
{
	poll_timer.function = vr4181a_pcmcia_poll_event;
	poll_timer.expires = jiffies + VR4181A_PCMCIA_POLL_PERIOD;
	add_timer(&poll_timer);
	schedule_task(&vr4181a_pcmcia_task);
}


/* 
 * vr4181a_pcmcia_interrupt()
 * The actual interrupt work is performed by vr4181a_pcmcia_task(), 
 ...
 */
static void 
vr4181a_pcmcia_interrupt(int irq, void *dev, struct pt_regs *regs)
{
	schedule_task(&vr4181a_pcmcia_task);
}
...
static int 
vr4181a_pcmcia_register_callback(unsigned int sock, 
 ...
 */
static int vr4181a_pcmcia_inquire_socket(unsigned int sock, socket_cap_t *cap)
{
	struct pcmcia_irq_info irq_info;

	if(sock > socket_count){
		printk(KERN_ERR "vr4181a: socket %u not configured\n", sock);
		return -1;
	}
...
static int 
vr4181a_pcmcia_get_status(unsigned int sock, unsigned int *status)
{
	struct pcmcia_state state;
...
static int 
vr4181a_pcmcia_get_socket(unsigned int sock, socket_state_t *state)
{
...
static int 
vr4181a_pcmcia_set_socket(unsigned int sock, socket_state_t *state)
{
...
static int 
vr4181a_pcmcia_get_io_map(unsigned int sock, struct pccard_io_map *map)
{
	DEBUG(1, "vr4181a_pcmcia_get_io_map: sock %d\n", sock);
...
int 
vr4181a_pcmcia_set_io_map(unsigned int sock, struct pccard_io_map *map)
{
	unsigned int speed;
	unsigned long start;
	...
	if(map->flags&MAP_ACTIVE){
		speed=(map->speed>0)?map->speed:VR4181A_PCMCIA_IO_SPEED;
...
static int 
vr4181a_pcmcia_get_mem_map(unsigned int sock, struct pccard_mem_map *map)
{
...
static int 
vr4181a_pcmcia_set_mem_map(unsigned int sock, struct pccard_mem_map *map)
{
	unsigned int speed;
	unsigned long start;
	u_long flags;
	...
	if(map->flags&MAP_ACTIVE){
		speed=(map->speed>0)?map->speed:VR4181A_PCMCIA_MEM_SPEED;
...
static void 
vr4181a_pcmcia_proc_setup(unsigned int sock, struct proc_dir_entry *base)
{
	...
	entry->read_proc=vr4181a_pcmcia_proc_status;
	entry->data=(void *)sock;
}


/* vr4181a_pcmcia_proc_status()
 * Implements the /proc/bus/pccard/??/status file.
 *
 * Returns: the number of characters added to the buffer
 */
static int 
vr4181a_pcmcia_proc_status(char *buf, char **start, off_t pos, 
