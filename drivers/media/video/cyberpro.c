...
/*
 * virtual address of CyberPro registers
 */
...
struct win_info {
	void (*init)(struct win_info *wi);
	void (*set_src)(struct win_info *wi);
	void (*set_win)(struct win_info *wi);
	void (*ctl)(struct win_info *wi, int on_off);
...
static void cyberpro_grphw8(int reg, unsigned int val)
{
	cyber2000_grphw(reg, val);
}

static unsigned char cyberpro_grphr8(int reg)
{
	cyber2000_outb(reg, 0x3ce);
	return cyber2000_inb(0x3cf);
}

static void cyberpro_grphw16(int reg, unsigned int val)
{
	cyber2000_grphw(reg, val);
	cyber2000_grphw(reg + 1, val >> 8);
}

static void cyberpro_grphw24(int reg, unsigned int val)
{
	cyber2000_grphw(reg, val);
	cyber2000_grphw(reg + 1, val >> 8);
	cyber2000_grphw(reg + 2, val >> 16);
}
...
static void
cyber2000fb_i2c_setlines(struct i2c_bus *bus, int ctrl, int data)
{
	unsigned long flags;
	int v;
...
static int
cyber2000fb_i2c_getdataline(struct i2c_bus *bus)
{
	unsigned long flags;
	int v;
	...
	cyber2000_outb(0xb6, 0x3ce);
	v = cyber2000_inb(0x3cf);
...
static void
cyber2000fb_i2c_attach(struct i2c_bus *bus, int id)
{
...
static void
cyber2000fb_i2c_detach(struct i2c_bus *bus, int id)
{
	struct cyberpro_grab_info *dp = (struct cyberpro_grab_info *)bus->data;
...
static struct i2c_bus cyberpro_bus = {
	"",
	I2C_BUSID_CYBER2000,
	NULL,
	...
	cyber2000fb_i2c_attach,
	cyber2000fb_i2c_detach,

	cyber2000fb_i2c_setlines,
	cyber2000fb_i2c_getdataline,
	NULL,
	NULL
};
 ...
 */
static void
cyberpro_ext_init(struct win_info *wi)
{
	...
	cyberpro_grphw8 (EXT_VID_DISP_CTL1,  wi->vid_disp_ctl1);
	cyberpro_grphw16(EXT_DDA_X_INIT,     0x0800);
	cyberpro_grphw16(EXT_DDA_Y_INIT,     0x0800);
	cyberpro_grphw16(EXT_VID_FIFO_CTL,   wi->vid_fifo_ctl);
	cyberpro_grphw8 (EXT_VID_FIFO_CTL1,  wi->vid_fifo_ctl1);
}
 ...
 */
static void
cyberpro_ext_set_src(struct win_info *wi)
{
	...
	cyberpro_grphw24(EXT_MEM_START, wi->src.offset);
	cyberpro_grphw16(EXT_SRC_WIDTH, pitch | ((phase << 4) & 0xf000));
	cyberpro_grphw8 (EXT_SRC_WIN_WIDTH, phase);
	cyberpro_grphw8 (EXT_VID_FMT, wi->vid_fmt);
}
 ...
 */
static void
cyberpro_ext_set_win(struct win_info *wi)
{
	...
	cyberpro_grphw16(EXT_X_START, xoff);
	cyberpro_grphw16(EXT_X_END,   xoff + wi->dst.width);
	cyberpro_grphw16(EXT_Y_START, yoff);
	cyberpro_grphw16(EXT_Y_END,   yoff + wi->dst.height);
	cyberpro_grphw24(EXT_COLOUR_COMPARE, wi->dst.chromakey);
	cyberpro_grphw16(EXT_DDA_X_INC, xscale);
	cyberpro_grphw16(EXT_DDA_Y_INC, yscale);
	cyberpro_grphw8(EXT_VID_FMT, wi->vid_fmt);
}
 ...
 */
static void
cyberpro_ext_ctl(struct win_info *wi, int on)
{
	if (on)
 ...
 */
static void
cyberpro_v2_init(struct win_info *wi)
{
	wi->vid_fifo_ctl  = 0xf87c;
	wi->vid_fmt       = EXT_VID_FMT_YUV422;
	wi->vid_disp_ctl1 = EXT_VID_DISP_CTL1_VINTERPOL_OFF |
	...
	cyberpro_grphw8(REG_BANK, REG_BANK_Y);
	cyberpro_grphw8 (Y_V2_VID_DISP_CTL1, wi->vid_disp_ctl1);
	/* No DDA init values */
	cyberpro_grphw16(Y_V2_VID_FIFO_CTL,  wi->vid_fifo_ctl);
	cyberpro_grphw8 (Y_V2_VID_FIFO_CTL1, wi->vid_fifo_ctl1);
}
 ...
 */
static void
cyberpro_v2_set_src(struct win_info *wi)
{
	unsigned int phase, pitch;
	...
	cyberpro_grphw8(REG_BANK, REG_BANK_X);
	cyberpro_grphw24(X_V2_VID_MEM_START, wi->src.offset);
	cyberpro_grphw16(X_V2_VID_SRC_WIDTH, pitch | ((phase << 4) & 0xf000));
	cyberpro_grphw8 (X_V2_VID_SRC_WIN_WIDTH, phase);

	cyberpro_grphw8(REG_BANK, REG_BANK_Y);
	cyberpro_grphw8(Y_V2_VID_FMT, wi->vid_fmt);
}
 ...
 */
static void
cyberpro_v2_set_win(struct win_info *wi)
{
	unsigned int xscale, yscale;
	unsigned int xoff, yoff;
	...
	cyberpro_grphw8(REG_BANK, REG_BANK_X);
	cyberpro_grphw16(X_V2_X_START,	xoff);
	cyberpro_grphw16(X_V2_X_END,	xoff + wi->dst.width);
	cyberpro_grphw16(X_V2_Y_START,	yoff);
	cyberpro_grphw16(X_V2_Y_END,	yoff + wi->dst.height);

	cyberpro_grphw8(REG_BANK, REG_BANK_Y);
	cyberpro_grphw16(Y_V2_DDA_X_INC, xscale);
	cyberpro_grphw16(Y_V2_DDA_Y_INC, yscale);
}
 ...
 */
static void
cyberpro_v2_ctl(struct win_info *wi, int on)
{
	if (on)
		wi->vid_disp_ctl1 |= EXT_VID_DISP_CTL1_ENABLE_WINDOW;
	else
 ...
 */
static void
cyberpro_x2_init(struct win_info *wi)
{
	wi->vid_fmt       = EXT_VID_FMT_YUV422;
	wi->vid_disp_ctl1 = 0x40;
	wi->vid_misc_ctl1 = 0;

	cyberpro_grphw8(REG_BANK, REG_BANK_K);
	cyberpro_grphw8 (K_X2_VID_DISP_CTL1, wi->vid_disp_ctl1);
	cyberpro_grphw16(K_X2_DDA_X_INIT,    0x0800);
	cyberpro_grphw16(K_X2_DDA_Y_INIT,    0x0800);
}
 ...
 */
static void
cyberpro_x2_set_src(struct win_info *wi)
{
	unsigned int phase, pitch;
	...
	cyberpro_grphw8(REG_BANK, REG_BANK_J);
	cyberpro_grphw24(J_X2_VID_MEM_START, wi->src.offset);
	cyberpro_grphw16(J_X2_VID_SRC_WIDTH, pitch | ((phase << 4) & 0xf000));
	cyberpro_grphw8 (J_X2_VID_SRC_WIN_WIDTH, phase);

	cyberpro_grphw8(REG_BANK, REG_BANK_K);
	cyberpro_grphw8(K_X2_VID_FMT, wi->vid_fmt);
}
 ...
 */
static void
cyberpro_x2_set_win(struct win_info *wi)
{
	unsigned int xscale, yscale;
	unsigned int xoff, yoff;
	...
	cyberpro_grphw8(REG_BANK, REG_BANK_J);
	cyberpro_grphw16(J_X2_X_START,	xoff);
	cyberpro_grphw16(J_X2_X_END,	xoff + wi->dst.width);
	cyberpro_grphw16(J_X2_Y_START,	yoff);
	cyberpro_grphw16(J_X2_Y_END,	yoff + wi->dst.height);

	cyberpro_grphw8(REG_BANK, REG_BANK_K);
	cyberpro_grphw16(K_X2_DDA_X_INC, xscale);
	cyberpro_grphw16(K_X2_DDA_Y_INC, yscale);
}
 ...
 */
static void
cyberpro_x2_ctl(struct win_info *wi, int on)
{
	if (on)
		wi->vid_disp_ctl1 |= EXT_VID_DISP_CTL1_ENABLE_WINDOW;
	else
...
static void cyberpro_interrupt(int nr, void *dev_id, struct pt_regs *regs)
{
	...
	if (1/* capture irq active */) {
		 ...
		 * Disable capture IRQ
		 */
		cyberpro_grphw8(VFAC_CTL3, dp->vfac3 & ~VFAC_CTL3_CAP_IRQ);
		 ...
		 */
		cyberpro_grphw8(VFAC_CTL3, dp->vfac3);
	}
...
{
	unsigned char ext_mem_ctl;

	cyber2000_outb(0x70, 0x3ce);
	ext_mem_ctl = cyber2000_inb(0x3cf);

	cyber2000_outb(ext_mem_ctl | 0x80, 0x3cf);
	cyber2000_outb(ext_mem_ctl, 0x3cf);
}
...
cyberpro_capture(struct cyberpro_grab_info *dp, int on)
{
	 ...
	 */
	old = cyberpro_grphr8(0xb5);

	if (on) {
		dp->vfac1 |= VFAC_CTL1_CAPTURE;

		cyberpro_grphw8(0xb5, old | 0x01);
...
cyberpro_capture_set_win(struct cyberpro_grab_info *dp)
{
	...
	cyberpro_grphw16(CAP_X_START, xstart);
	cyberpro_grphw16(CAP_X_END, xend + 1);
	cyberpro_grphw16(CAP_Y_START, ystart);
	cyberpro_grphw16(CAP_Y_END, yend + 2);

	/*
	 * This should take account of capt.decimation
	 */
	cyberpro_grphw16(CAP_DDA_X_INIT, 0x0800);
	cyberpro_grphw16(CAP_DDA_X_INC, 0x1000);
	cyberpro_grphw16(CAP_DDA_Y_INIT, 0x0800);
	cyberpro_grphw16(CAP_DDA_Y_INC, 0x1000);

	cyberpro_grphw8(CAP_PITCH, dp->capt.width >> 2);
	cyberpro_grphw8(CAP_MODE1, dp->cap_mode1);
}
...
cyberpro_set_interlace(struct cyberpro_grab_info *dp)
{
	...
	cyberpro_grphw8(VFAC_CTL3,    dp->vfac3);
	cyberpro_grphw8(CAP_CTL_MISC, dp->cap_miscctl);

	dp->ovl->set_src(dp->ovl);

	if (dp->win_set)
		dp->ovl->set_win(dp->ovl);
}
...
#define MAX_SIZE	(maxbufsz + 16384)
	...
	if ((b->height * b->bytesperline + MAX_SIZE) >= fb_size)
		return -EINVAL;
	...
	cyberpro_grphw24(CAP_MEM_START, dp->cap_mem_offset);
...
static void
cyberpro_init(struct cyberpro_grab_info *dp)
{
	/*
	 ...
	 */
	cyberpro_grphw8(BM_CTRL1, 0x88);
	cyberpro_grphw8(BIU_BM_CONTROL, BIU_BM_CONTROL_ENABLE);
	cyberpro_grphw8(BM_CTRL0, 0x44);
	cyberpro_grphw8(BM_CTRL1, 0x84);
	...
	cyberpro_grphw8(VFAC_CTL1, dp->vfac1);
	cyberpro_grphw8(VFAC_CTL3, dp->vfac3);
	cyberpro_grphw8(VFAC_CTL2, dp->vfac2);

	cyberpro_grphw8(REG_BANK, REG_BANK_Y);		/* register bank Y */
	cyberpro_grphw8(Y_TV_CTL, 0x80);		/* reg Y */
	...
	cyberpro_grphw8(CAP_NEW_CTL1, 0x3f);	/* disable PIP */
	cyberpro_grphw8(CAP_NEW_CTL2, 0xc0);	/* no IRQs */
	cyberpro_grphw8(CAP_MODE2,    0x50);	/* normal */

	/* setup overlay */
	cyberpro_grphw16(EXT_FIFO_CTL, 0x1010);
...
cyberpro_deinit(struct cyberpro_grab_info *dp)
{
	 ...
	 */
	cyberpro_grphw8(BIU_BM_CONTROL, 0);
...
cyberpro_grabber_read(struct video_device *dev, char *buf, unsigned long count, int noblock)
{
	...
	char *disp = fb_base + (dp->cap_mem_offset << 2);
...
void cleanup_module(void)
{
	...
	i2c_unregister_bus(&cyberpro_bus);
