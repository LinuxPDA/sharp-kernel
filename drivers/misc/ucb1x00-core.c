 ...
 *	synchronised ADC conversions (via the ADCSYNC pin) are not
 *	currently supported.
 *
...
static void ucb1x00_configure(struct ucb1x00 *ucb)
{
	...
	if (machine_is_adsbitsy()) {
		ucb->irq = SA1111_IRQ(14);
	}
#ifdef CONFIG_SA1100_ASSABET
	if (machine_is_assabet()) {
		ucb->irq = ASSABET_IRQ_GPIO_UCB1300_IRQ;
		irq_gpio_pin = ASSABET_GPIO_UCB1300_IRQ;
	}
#endif
