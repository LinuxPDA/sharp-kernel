 ...
 * likely mean updating the networking layer.
 *
 ...
 * 25-mar-2001	More SA-110 updates, including workaround for ip problem
 ...
 * support (arch/arm/sa1100/usb-eth.c).
 *
 *-------------------------------------------------------------------------*/
...
static void nc_dump_registers (struct usbnet *dev)
{
	...
	for (reg = 0; reg < 0x20; reg++) {
		...
		retval = nc_register_read (dev, reg, &value);
		if (retval < 0)
...
static int net1080_reset (struct usbnet *dev)
{
	...
	if ((retval = nc_register_read (dev, REG_STATUS, &status)) < 0) {
		dbg ("can't read dev %d status: %d", dev->udev->devnum, retval);
		goto done;
	}
	// nc_dump_status (dev, status);

	if ((retval = nc_register_read (dev, REG_USBCTL, &usbctl)) < 0) {
	...
	if ((retval = nc_register_read (dev, REG_TTL, &ttl)) < 0) {
...
static void rx_submit (struct usbnet *dev, struct urb *urb, int flags)
{
	...
	/* make the IP layer happier with 16-byte alignment.
	 * Ethernet header uses 14 bytes; pad 2 normally.
	 * NetChip framing adds another 6, so pad total of 12.
	 */
	size = (dev->driver_info->flags & FLAG_FRAMING)
			? (12 + FRAMED_SIZE (dev->net.mtu))
			: (2 + sizeof (struct ethhdr) + dev->net.mtu);
	...
	skb_reserve (skb, (dev->driver_info->flags & FLAG_FRAMING) ? 12 : 2);
...
static int unlink_urbs (struct sk_buff_head *q)
{
	...
	for (skb = q->next; skb != (struct sk_buff *) q; skb = skbnext) {
		...
		// these unlinks complete immediately
...
MODULE_DESCRIPTION ("USB Host-to-Host Link Drivers (Linux, NetChip, Prolific, ...)");
