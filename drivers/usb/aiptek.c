...
#define DRIVER_DESC "USB Aiptek 8000U tablet driver"
 ...
 * 0x04/0x00	GerFirmwareVersion	2	Firmware Version
 * 
 *
...
static void aiptek_irq(struct urb *urb)
{
	...
	static int serial = 0;  // Mega-hack
	int p;

	if (urb->status) return;

	// in range
	input_report_key(dev, BTN_TOOL_PEN, data[5] & 1);

	if (data[0] != 2)
		dbg("received unknown report #%d", data[0]);

	input_report_abs(dev, ABS_X, data[1] | ((__u32)data[2] << 8));
	input_report_abs(dev, ABS_Y, data[3] | ((__u32)data[4] << 8));

	p = (data[6] | ((__u32)data[7] << 8));
	p -= aiptek->features->pressure_min;
	if(p < 0)
		p = 0;
	...
	input_report_abs(dev, ABS_MISC, serial ^= 1);
}

struct aiptek_features aiptek_features[] = {
	{ "Aiptek 8000U",      8, 3000,  2250,  26, 511, aiptek_irq,
		0, 0, 0, 0 },
	{ NULL , 0 }
};
...
static int aiptek_open(struct input_dev *dev)
{
...
static void aiptek_close(struct input_dev *dev)
{
	struct aiptek *aiptek = dev->private;
...
static void aiptek_command(struct usb_device *dev, unsigned int ifnum, unsigned char command, unsigned char data)
{
	__u8 buf[3];
...
static void *aiptek_probe(struct usb_device *dev, unsigned int ifnum, const struct usb_device_id *id)
{
	...
	aiptek->dev.absmax[ABS_PRESSURE] = aiptek->features->pressure_max - aiptek->features->pressure_min;
	...
	FILL_INT_URB(&aiptek->irq, dev, usb_rcvintpipe(dev,
			endpoint->bEndpointAddress), aiptek->data,
			aiptek->features->pktlen, aiptek->features->irq, aiptek,
			endpoint->bInterval);
...
static void aiptek_disconnect(struct usb_device *dev, void *ptr)
{
