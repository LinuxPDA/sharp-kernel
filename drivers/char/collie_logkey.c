...
# if defined(IRIS_KBD_LANGUAGE_ENGLISH)
#  define RAW_MAP "iris_rawmap_en.h"
# else /* language selection , default */
...
//printk("kbd_press=%d\n",keycode);
  ...
  if (newpress){
    ...
    init_timer(&sharppda_kbd_rep_timer);
#ifdef CONFIG_IRIS
    /* Disable 'DISP' key-repeat */
    if( keycode_mapped == SLKEY_SCREEN ){
      return;
    }
    /* Disable 'Frontlight' key-repeat */
    if( keycode_mapped == SLKEY_FRONTLIGHT ){
      return;
    }
#endif /* CONFIG_IRIS */
    sharppda_kbd_rep_timer.expires = jiffies + key_repeat_delay;
...
#ifdef CONFIG_IRIS
#define INVALID_MULTIKEY_PRESS 3
#endif /* CONFIG_IRIS */
...
void sharppda_scan_key_gpio(void* dummy)
{
...
#ifdef CONFIG_IRIS
  {
    ...
    if( iris_kbdctl_keyboard_disabled == 1 ){
...
unsigned long sharppda_kbd_extmodif_packstatus(void)
{
  ...
  for(i=0;i<EXTMODIF_SHOWSTAT_KEYMAX;i++){
    v = sharppda_kbd_extmodif_showstatus(i);
    if( v < 0 ) v = 0;
    val |= ( ( v ? 1 : 0 ) << i );
  }
  return val;
  ...
  for(i=0;i<EXTMODIF_SHOWSTAT_KEYMAX;i++){
    ...
    if( pmodmap != NULL && pmodmap[sharp_cur_status] ){
      val |= ( 1 << i );
    }
  }
#endif
