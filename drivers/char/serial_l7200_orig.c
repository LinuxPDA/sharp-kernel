...
void l7200_rs_enable_irq(int dev, int dir)
{
	...
	__IOB(up->regbase + UARTINTMASK) |= dir;
...
void l7200_rs_disable_irq(int dev, int dir)
{
	...
	__IOB(up->regbase + UARTINTMASK) &= ~dir;
	if (!(__IOB(up->regbase+UARTINTMASK) | UART_TXINT | UART_RXINT)) {
		disable_irq(up->irq);
	}
}
...
static void l7200_rs_interrupt(int irq, void *dev_id, struct pt_regs* regs)
{
	...
	int irqst = __IOB(up->regbase + UARTINTSTAT);
	...
	if (irqst & UART_RXINT) {
		...
		while (!(__IOB(up->regbase + UARTFLG) & UARTFLG_URXFE)) {
			unsigned char ch = __IOB(up->regbase + UARTDR);
			unsigned char st = __IOB(up->regbase + RXSTAT);
			char flag = 0;
	...
	if (irqst & UART_TXINT) {
		while (!(__IOB(up->regbase + UARTFLG) & UARTFLG_UTXFF)) {
			if (up->xchar) {
				__IOB(up->regbase + UARTDR) = up->xchar;
				up->xchar = 0;
				continue;
			}
			if (up->putp == up->getp) {
			...
			__IOB(up->regbase + UARTDR) = *up->getp;
...
static inline void l7200_rs_set_cflag(int dev, int cflag)
{
	...
	if ( up->regbase ) {
		__IOB(up->regbase + L_UBRLCR) = cr_l & 0xFF;
		__IOB(up->regbase + H_UBRLCR) = cr_h & 0xFF;
	}
	else {
...
static void l7200_rs_wait_until_sent(struct tty_struct* tty, int timeout)
{
	...
	while (__IOB(up->regbase + UARTFLG) & UARTFLG_UBUSY) {
...
static int l7200_rs_open(struct tty_struct* tty, struct file *filp)
{
	...
	up->flag++;
	__IOB(up->regbase + UARTCON) |= UARTCON_UARTEN;
...
static int __init l7200_rs_init(void)
{
	...
	l7200_rs_driver.hangup          = NULL;
	l7200_rs_driver.break_ctl       = NULL;
