...
static char *serial_name = "SharpSL serial driver";
...
#if defined(CONFIG_ARCH_SHARP_SL)	// H.Hayami SHARP 2002.5.1
...
#define xpa2X0_is_bt_uart(iomem)	((iomem) == (void *)&BTUART)
#define xpa2X0_is_st_uart(iomem)	((iomem) == (void *)&STUART)
...
#else
#define xpa210_discovery_irda_power_on()	set_discovery_irda(0x01)
#define xpa210_discovery_irda_power_off()	clr_discovery_irda(0x01)
#endif
...
#if defined(CONFIG_ARCH_SHARP_SL)	//SHARP 2002.5.4

#define STUART_IER	STIER
#define STUART_MCR	STMCR
#define STUART_ISR	STISR
...
static _INLINE_ void transmit_chars(struct async_struct *info, int *intr_done)
{
	...
	if (info->x_char) {
#if defined(CONFIG_ARCH_SHARP_SL)	// H.Hayami SHARP 2002.5.2
		if (xpa2X0_is_st_uart(info->iomem_base)) {
...
static int startup(struct async_struct * info)
{
	...
	{
		if (state->irq != 0)
#if defined(CONFIG_ARCH_SHARP_SL) && (CONFIG_XPA2x0_A1)
			{if(info->io_type == SERIAL_IO_MEM)
			info->MCR &= ~UART_MCR_OUT2;
...
//close IR	
#ifdef CONFIG_PM
#if defined(CONFIG_ARCH_SHARP_SL) && defined(CONFIG_IRDA)
	if (xpa2X0_is_st_uart(info->iomem_base)) {
		ICMR &= ~(1 << 20); //stuart
...
#if defined(CONFIG_SERIAL_CONSOLE) && defined(CONFIG_ARCH_SHARP_SL)
		if( info->io_type == SERIAL_IO_MEM ){
		if (!(info->flags & ASYNC_CALLOUT_ACTIVE) &&
		    !(info->flags & ASYNC_CLOSING))
			break;
...
#if defined(CONFIG_IRDA) && defined(CONFIG_ARCH_SHARP_SL)
...
#ifdef CONFIG_PM
#define BTUART_MCR	BTMCR
#define FFUART_MCR	FFMCR
...
#if defined(CONFIG_SABINAL_DISCOVERY) && (CONFIG_XPA2x0_A1)
...
#if !defined(CONFIG_SABINAL_DISCOVERY) && !defined(CONFIG_ARCH_PXA_TOSA) && !defined(CONFIG_ARCH_PXA_SPITZ)
