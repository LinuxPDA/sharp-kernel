 ...
 *
 *  6/02: Change file name from 'serial.c' to 'serial_pxa200.c'
...
#else
#include <asm/arch/serial_pxa200.h>
...
static char *serial_name = "Discovery serial driver";
...
void clr_discovery_serial(void){
	ASIC3_GPIO_PIOD_B &= ~(0x2000);
}
void set_discovery_serial(void){
	ASIC3_GPIO_PIOD_B |= 0x2000;
}
#define xpa210_discovery_serial_power_off()	clr_discovery_serial()
#define xpa210_discovery_serial_power_on()	set_discovery_serial()

#if defined(CONFIG_ARCH_COTULLA) && (CONFIG_SABINAL_DISCOVERY)

#define CONFIG_DISCOVERY_DVT

#include <asm/arch/power_consumption.h>

#define xpa2X0_is_ff_uart(iomem)	((iomem) == COTULLA_UART1_BASE)
#define xpa2X0_is_bt_uart(iomem)	((iomem) == COTULLA_UART2_BASE)
#define xpa2X0_is_st_uart(iomem)	((iomem) == COTULLA_UART3_BASE)

#define XPA2X0_UART_ISR			8	// Infrared Selection Register

#define XPA2X0_UART_ISR_RXPL		0x10
#define XPA2X0_UART_ISR_TXPL		0x08
#define XPA2X0_UART_ISR_XMODE		0x04
#define XPA2X0_UART_ISR_RCVEIR		0x02
#define XPA2X0_UART_ISR_XMITIR		0x01

#define XPA210_DISCOVERY_IR_STOP	(XPA2X0_UART_ISR_RXPL | XPA2X0_UART_ISR_XMODE)
#define XPA210_DISCOVERY_IR_RECV	(XPA2X0_UART_ISR_RXPL | XPA2X0_UART_ISR_XMODE | XPA2X0_UART_ISR_RCVEIR)
#define XPA210_DISCOVERY_IR_SEND	(XPA2X0_UART_ISR_RXPL | XPA2X0_UART_ISR_XMODE | XPA2X0_UART_ISR_XMITIR)
...
void set_discovery_irda(u8 x){
	ASIC3_GPIO_PIOD_B |= 0x1000;
	//printk("discovery_irda power off\n");
}

#define xpa210_discovery_irda_power_on()	clr_discovery_irda(0x01)
#define xpa210_discovery_irda_power_off()	set_discovery_irda(0x01)

static int irda_open = 0;
struct async_struct *irda_info = 0;
...
//			printk("serial_in: read[0x%08x]\n",
//			       (unsigned long) info->iomem_base + (offset<<info->iomem_reg_shift));
...
#ifdef CONFIG_ARCH_COTULLA
		if (xpa2X0_is_st_uart(info->iomem_base)) {
//			printk("serial_out: write[0x%08x]=0x%08x\n",
//			       (unsigned long) info->iomem_base + (offset<<info->iomem_reg_shift),value);
...
#if defined(CONFIG_ARCH_COTULLA) && (CONFIG_SABINAL_DISCOVERY)	//SHARP 2002.5.4

#define STUART_IER	*(COTULLA_UART3_BASE + UART_IER*4)
#define STUART_MCR	*(COTULLA_UART3_BASE + UART_MCR*4)
#define STUART_ISR	*(COTULLA_UART3_BASE + 8*4)

void xpa210_discovery_irda_resume () {
	
		CKEN |= 0x20; //stuart

		STUART_IER &= ~(UART_IER_UUE);
		
		STUART_MCR = 0x08; 
		
		STUART_ISR = XPA210_DISCOVERY_IR_RECV;
		
		STUART_IER |= UART_IER_UUE;	/* enable UUE */

		//printk("xpa210_discovery_irda_resume\n");
	
}

void xpa210_discovery_irda_enable(struct async_struct * info, u8 x){
	...
	if (x == 0) {
		...
		serial_out(info, XPA2X0_UART_ISR, XPA210_DISCOVERY_IR_STOP);
		...
		serial_out(info, XPA2X0_UART_ISR, XPA210_DISCOVERY_IR_RECV);
		
		info->IER |= UART_IER_UUE;	/* enable UUE */
		serial_out(info, UART_IER, info->IER);

		//printk("xpa210_discovery_irda_enable\n");
	}
...
static _INLINE_ void transmit_chars(struct async_struct *info, int *intr_done)
{
	...
	if (info->x_char) {
#if defined(CONFIG_SABINAL_DISCOVERY)	// H.Hayami SHARP 2002.5.2
		if (xpa2X0_is_st_uart(info->iomem_base)) {
			serial_outp(info, XPA2X0_UART_ISR, XPA210_DISCOVERY_IR_SEND);
			printk("x_char\n");
		}
#endif
...
//		 	printk("%x", info->xmit.buf[info->xmit.tail]);
...
static void rs_interrupt_single(int irq, void *dev_id, struct pt_regs * regs)
{
	...
	if (xpa2X0_is_st_uart(info->iomem_base)) {
		...
		do {
			...
			status = serial_inp(info, UART_IIR);
			...
			} else if (status & UART_IIR_RDI) {
				receive_chars(info, &status, regs);
				
			} else {

//				if (status0 & UART_LSR_DR)
//					receive_chars(info, &status, regs);
//				if (status0 & UART_LSR_THRE) {
//					serial_outp(info, XPA2X0_UART_ISR, XPA210_DISCOVERY_IR_SEND);
//					transmit_chars(info, 0);
//				}	
			}
			
			if (status & UART_IIR_MSI)
				check_modem_status(info);

			if (status & UART_IIR_RLSI)
				status0 = serial_inp(info, UART_LSR);
		
//			if (pass_counter++ > RS_ISR_PASS_LIMIT) {
//				break;
//			}

		} while (!(status & UART_IIR_NO_INT));
...
static int startup(struct async_struct * info)
{
	...
	{
		if (state->irq != 0)
#if defined (CONFIG_ARCH_COTULLA) && (CONFIG_XPA2x0_A1)
			{if(info->io_type == SERIAL_IO_MEM)
			info->MCR &= ~UART_MCR_OUT2;
			else info->MCR |= UART_MCR_OUT2;}
#else
			info->MCR |= UART_MCR_OUT2;
#endif
	}
	...
	}else serial_outp(info, UART_IER, info->IER);
#else
	serial_outp(info, UART_IER, info->IER);	/* enable interrupts */
#endif
			 ...
			 struct termios *old_termios)
{
	...
	if (xpa2X0_is_st_uart(info->iomem_base)) {
		STISR = serial_in(info, XPA2X0_UART_ISR);
//		printk("IRDA: change_speed, STISR=%x ",STISR);
//		serial_outp(info, XPA2X0_UART_ISR, XPA210_DISCOVERY_IR_STOP);
	}
...
//		printk(" cval=%x, baud_base=%x, quot=%x\n",cval,info->state->baud_base,quot);
		...
		serial_outp(info, XPA2X0_UART_ISR, STISR);
		    ...
		    const unsigned char *buf, int count)
{
	...
	if (xpa2X0_is_st_uart(info->iomem_base)) {
		...
		save_flags(flags); cli();
		rs_interrupt_single(info->state->irq, (void *)1, NULL);
		restore_flags(flags);
...
#if defined (CONFIG_ARCH_COTULLA) && (CONFIG_XPA2x0_A1)
			     | ((arg & TIOCM_OUT2) ? 0 : UART_MCR_OUT2)
#else
...
#if defined (CONFIG_ARCH_COTULLA) && (CONFIG_DISCOVERY_EVT2)
...
#if defined(CONFIG_SERIAL_CONSOLE) && defined(CONFIG_ARCH_COTULLA)
		if( info->io_type == SERIAL_IO_MEM ){
		if (!(info->flags & ASYNC_CALLOUT_ACTIVE) &&
		    !(info->flags & ASYNC_CLOSING))
			break;
...
#if defined(CONFIG_IRDA) && defined(CONFIG_SABINAL_DISCOVERY)
	if (xpa210_discovery_is_irda_line(line)) {
		info->IER = 0x01;
		xpa210_discovery_irda_enable(info, 1);
		xpa210_discovery_irda_power_on();
		ICMR |= (1 << 20); //btuart
		irda_open = 1;
		irda_info = info;
		set_driver_waste_bits(IRDA_WASTE);
	}
#endif
...
static unsigned detect_uart_irq (struct serial_state * state)
{
	...
	else serial_outp(&scr_info, UART_MCR, UART_MCR_OUT1 | UART_MCR_OUT2);
#else
	serial_outp(&scr_info, UART_MCR, UART_MCR_OUT1 | UART_MCR_OUT2);
#endif
...
#ifdef CONFIG_PM
#define BTUART_MCR	*(unsigned char *)(COTULLA_UART2_BASE + 0x10)
#define STUART_MCR	*(unsigned char *)(COTULLA_UART3_BASE + 0x10)
...
//	ASIC3_GPIO_PIOD_C = (ASIC3_GPIO_PSTS_C | 0x2000);
...
//	ASIC3_GPIO_PIOD_C = (ASIC3_GPIO_PSTS_C & ~(0x2000));
	...
	if ( irda_open == 1 ) {
		...
		xpa210_discovery_irda_enable(irda_info, 1);
		xpa210_discovery_irda_power_on();
		ICMR |= (1 << 20); //btuart
	}
#endif
...
static int cotulla_serial_pm_callback(struct pm_dev* pm_dev,
			       pm_request_t req, void* data)
{
	switch (req) {
	case PM_SUSPEND:
		cotulla_serial_suspend();
		break;
		
	case PM_RESUME:
		cotulla_serial_resume();
		break;
 ...
 */
static int __init pxa200_rs_init(void)
{
	int i;
	struct serial_state * state;
	int retval2;
...
#else
	serial_driver.driver_name = "serial_cpu";
#endif
#endif
...
#if defined(CONFIG_SABINAL_DISCOVERY)
			if (xpa2X0_is_st_uart(state->iomem_base)) {
...
#if defined(CONFIG_IRDA) && defined(CONFIG_SABINAL_DISCOVERY)
	printk("rs_init: IRDA H/W initializaion\n");
 	...
 	GPDR(1) = v2;
		...
		//init DCD of ASIC3
		ASIC3_GPIO_MASK_D &= ~COM_DCD; //unmask
		ASIC3_GPIO_DIR_D &= ~COM_DCD; //input
		ASIC3_GPIO_INTYP_D |= COM_DCD; //edge trigger

		if (ASIC3_GPIO_PSTS_D &  COM_DCD) {
			printk("DCD falling edge trigger\n");
			ASIC3_GPIO_ETSEL_D &= ~(COM_DCD); //falling edge trigger
		} else {
			printk("DCD raising edge trigger\n");
			ASIC3_GPIO_ETSEL_D |= (COM_DCD); //raising edge trigger
		}
//		retval2 = request_irq(IRQ_ASIC3_COM_DCD, rs_interrupt_DCD, SA_INTERRUPT,
//				     "serial_DCD", rs_interrupt_DCD);

		printk("DCD initial done\n"); 
#endif

#ifdef CONFIG_PM
//	serial_pm_dev = pm_register(PM_SYS_DEV, 0, cotulla_serial_pm_callback);
	serial_pm_dev = pm_register(PM_COTULLA_DEV, 0, cotulla_serial_pm_callback);
#endif
...
static void __exit pxa200_rs_fini(void) 
{
	unsigned long flags;
	int e1, e2;
	int i;
...
module_init(pxa200_rs_init);
module_exit(pxa200_rs_fini);
MODULE_DESCRIPTION("PXA2X0 discovery serial driver");
				...
				unsigned count)
{
	...
	else	serial_out(info, UART_IER, 0x00);
#else
...
static int __init serial_console_setup(struct console *co, char *options)
{
	...
	if( info->io_type == SERIAL_IO_MEM )
		serial_out(info, UART_IER, UART_IER_UUE);
	else	serial_out(info, UART_IER, 0);
#else
	serial_out(info, UART_IER, 0);
#endif
#ifdef CONFIG_DISCOVERY_DVT
