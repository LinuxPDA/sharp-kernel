/*
 *  linux/drivers/char/spitz_logkey.c
 *
...
#include <asm/arch/keyboard_spitz.h>
...
#define KEYMASK_SYNC		(0x1<<1)
#define KEYMASK_ADDRESS		(0x1<<3)
#define KEYMASK_CALENDAR	(0x1<<4)
#define KEYMASK_MAIL		(0x1<<5)
#define KEYMASK_HOME		(0x1<<6)
...
extern void spitz_kbd_discharge_all(void);
extern void spitz_kbd_charge_all(void);
extern void spitz_kbd_activate_col(int col);
extern void spitz_kbd_reset_col(int col);
extern void spitz_kbd_activate_all(void);
# define DISCHARGE_ALL            spitz_kbd_discharge_all()
# define CHARGE_ALL               spitz_kbd_charge_all()
# define WAIT_DISCHARGE_DELAY     udelay(KB_DISCHARGE_DELAY)
# define ACTIVATE_COL(c)          spitz_kbd_activate_col((c))
# define RESET_COL(c)             spitz_kbd_reset_col((c))
# define WAIT_ACTIVATE_DELAY      udelay(KB_ACTIVATE_DELAY)
# define GET_ROWS_STATUS(c)       ((GPLR0 >> 12) & 0x01) | ((GPLR0 >> 16) & 0x02) | ((GPLR2 >> 25) & 0x04) | ((GPLR1 << 1) & 0x08) | ((GPLR1 >> 0) & 0x10) | ((GPLR1 >> 1) & 0x60)
# define DRIVE_ALL_COLS           spitz_kbd_activate_all()
# define GET_REAL_COL_FROM_COL(c)    (c)
# define RAW_MAP "spitz_rawmap.h"
# define RAW_MAP_ROLLOVER "spitz_rawmap_rollover.h"
...
#define PORT_KEY_NUM	2
#define IDPORT_ON	0
#define IDPORT_SYNC	1

static int port_key_setting[PORT_KEY_NUM] = { 20, 5  }; // -1:disable, 0~: x10ms wait for detect
static int port_key_delay_state[PORT_KEY_NUM] = {NOTPRESSED,NOTPRESSED};
...
int set_port_key_setting( int num, int val )
{
  ...
  port_key_setting[idx] = val;
  port_key_delay_state[idx] = NOTPRESSED;
  port_key_pending = 0;
  return 0;
}
...
static int read_port_key_status_raw(void)
{
  ...
  // Power
  if ((GPLR(GPIO_ON_KEY) & GPIO_bit(GPIO_ON_KEY))!=0) val |= KEYMASK_ON;

  // Sync
  if ((GPLR(GPIO_SYNC) & GPIO_bit(GPIO_SYNC))!=0) val |= KEYMASK_SYNC;
...
void sharppda_scan_key_gpio(void* dummy)
{
	...
	for(col=0;col<KB_COLS+1;col++){
		...
		if (view_mode) { // Full key is disabled in view mode
		  if (col<10) continue;
		  rowd &= 0x3;
		}
	...
	if (count >= MAX_KEY_PRESS) {
		...
		{
		    int shift_l = ROLLOVER_CHECK_FLAG(rollover_flag, ROLLOVER_LSHIFT);
		    int shift_r = ROLLOVER_CHECK_FLAG(rollover_flag, ROLLOVER_RSHIFT);
		    if ( (shift_l && !shift_r) || (!shift_l && shift_r) ) {
			rollover_count--;
		    }
		}
	...
	if (spitz_initial_keypress<0) {
#if CHECK_CHUTTER_BOUNCE
		if ((sharppda_kbdstate[0x65].in==PRESSED) ||
		    IS_PRESSING(sharppda_kbdstate[0x65].in)) {
#else
#if CHECK_ROLLOVER
		if (sharppda_kbdstate[0x65].in==1) {
#else
		if (sharppda_kbdstate[0x65].in) {
#endif
#endif
			spitz_initial_keypress = 1;
		} else {
			spitz_initial_keypress = 0;
		}
	}
	 ...
	 */
#if CHECK_CHUTTER_BOUNCE
	for (loop = 1; loop < NR_KEYS; loop++) { // loop=0 is not used
		if (port_key_pending) {
...
u32 sharppda_kbd_is_wakeup(void)
{
  unsigned long flags;
  int count = 0, err = 0;
  int cur_portkey=0,last_portkey=0;
  int cur_matrixkey=0,last_matrixkey=0;
  const int strobe_num = 0;
  const int success_count = 10;
  u32 logical_wakeup_src_mask = get_logical_wakeup_src_mask();
  u32 retval = 0;
  int view_mode = read_hinge_state();
...
//    printk("key interrupt on (%04x,%04x)\n",last_portkey,last_matrixkey);
...
#if 1 // disable on-key when LCD is closed.
    if (read_hinge_state() == 0x3 && (last_portkey & KEYMASK_ON)) {
      //printk("disable on-key\n");
      last_portkey &= ~KEYMASK_ON;
    }
#endif
    if (last_portkey & KEYMASK_ON) { // On ... only status change
      sharppda_kbdstate[KEYCODE(0, KB_COLS)].in = PRESSED;
      port_key_delay_state[IDPORT_ON]=PRESSED;
      retval |= IDPM_WAKEUP_ONKEY;
    }
	...
	retval |= IDPM_WAKEUP_ADDRESSBOOK;
	...
	retval |= IDPM_WAKEUP_MAIL;
      } else last_matrixkey &= ~KEYMASK_MAIL;
    }
    if (last_matrixkey & KEYMASK_HOME) { // Home
      if (logical_wakeup_src_mask&IDPM_WAKEUP_HOME) {
	sharppda_kbd_press_repeat(KEYCODE(6, strobe_num));
	retval |= IDPM_WAKEUP_HOME;
      } else last_matrixkey &= ~KEYMASK_HOME;
    }
    return retval;
  }
  return 0;
