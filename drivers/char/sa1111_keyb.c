...
static unsigned int handle_kbd_event(void);
...
static unsigned int handle_kbd_event(void)
{
       ...
       while ((status & KBD_STAT_RXF) || (msests & MSE_STAT_RXF)) {
               while (status & KBD_STAT_RXF) {
                       scancode = KBDDATA & 0xff;
                       if (! (status & KBD_STAT_STP))
                               handle_keyboard_event(scancode);
                       if (!--work) {
                               printk(KERN_ERR "pc_keyb: keyboard controller jammed (0x%02X).\n", status);
                               return status;
                       }
                       status = KBDSTAT;
               }
               while (msests & MSE_STAT_RXF) {
                       ...
                       if (!--work) {
                               printk(KERN_ERR "pc_keyb: mouse controller jammed (0x%02X).\n", msests);
                               return status; /*XXX*/
                       }
                       msests = MSESTAT;
               }
...
#if defined CONFIG_PSMOUSE
static void mse_write_cmd(int cmd)
{
       unsigned long flags;
