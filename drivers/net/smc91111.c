/*------------------------------------------------------------------------
 ...
 . 	2003-01-14: Takeo Takahashi Support M32RUT-LAN(Rev.B)
 ----------------------------------------------------------------------------*/
...
#include <linux/in.h>
#if defined(__m32r__) || defined(__sh__)
...
#if defined(__m32r__) || defined(__sh__)
	for ( i = 0; i < 4 ; i+=2 ) {
		outw( (multicast_table[i+1] << 8) | multicast_table[i], ioaddr + MCAST_REG1 + i );
	}
#else
	for ( i = 0; i < 8 ; i++ ) {
...
#if defined(__m32r__) || defined(__sh__)
                outw(0x2000 + (buf[length - 1] & 0xff), ioaddr + DATA_REG);
#else
...
#endif
#if defined(CONFIG_SH_7760_SOLUTION_ENGINE) && defined(NO_AUTOPROBE)
	dev->irq = SH7760SE_IRQ_IRQ2;
#endif
...
static void smc_tx( struct net_device * dev )
{
	...
	/* maybe bug fixed. when the packet_no is masked by 0x7f,
	 * we can not check TXFIFO_TEMPTY flag correctly.
	 * 2003-06-30 by takeo
	 */
	packet_no &= 0x7F;
#else
