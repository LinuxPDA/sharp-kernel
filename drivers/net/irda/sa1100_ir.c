...
static inline int sa1100_irda_set_power_collie(struct sa1100_irda *si, int state)
{
	if (state) {
		/*
		 *  0 - off
 ...
 */
static inline int
sa1100_irda_set_power_bitsy(struct sa1100_irda *si, unsigned int state)
{
	if (state)
...
__sa1100_irda_set_power(struct sa1100_irda *si, unsigned int state)
{
	...
	if (machine_is_collie())
		ret = sa1100_irda_set_power_collie(si, state);
	if (machine_is_bitsy())
...
int __init sa1100_irda_init(void)
{
	...
	err = request_mem_region(_Ser2UTCR0, 0x24, "IrDA") ? 0 : -EBUSY;
	if (err)
		goto err_mem_1;
	err = request_mem_region(_Ser2HSCR0, 0x1c, "IrDA") ? 0 : -EBUSY;
	if (err)
		goto err_mem_2;
	err = request_mem_region(_Ser2HSCR2, 0x04, "IrDA") ? 0 : -EBUSY;
	if (err)
		goto err_mem_3;
	...
	if (err) {
		release_mem_region(_Ser2HSCR2, 0x04);
err_mem_3:
		release_mem_region(_Ser2HSCR0, 0x1c);
err_mem_2:
		release_mem_region(_Ser2UTCR0, 0x24);
	} else {
	 	printk("SA1100 IrDA initialized.\n");
	}
err_mem_1:
	return err;
}
