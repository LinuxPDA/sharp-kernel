...
#if defined(CONFIG_ARCH_DBMX2)
void set_GPIO_E_11_interrupt(void)
{
	_reg_GPIO_DDIR(GPIOE) &= ~(0x1<<11);
	_reg_GPIO_GIUS(GPIOE) |= 0x1<<11;
	_reg_GPIO_ICR1(GPIOE) &= ~0xC00000; // config as rising edge trigger 
	_reg_GPIO_IMR(GPIOE) |= 0x1<<11;
}

void    Clear_GPIO_E_11_interrupt(void)
{
	_reg_GPIO_ISR(GPIOE) |= 0x1<<11;
}
#endif
...

/**
 * ax88796_open - Open/initialize the board.
...
int ax88796_open(struct net_device *dev)
{
	...
	if (ei_local == NULL) 
	{
		printk(KERN_EMERG "%s: ax88796_open passed a non-existent device!\n", dev->name);
		return -ENXIO;
	}
	...
	ei_local->irqlock = 0;
#if defined(CONFIG_ARCH_DBMX2)
	set_GPIO_E_11_interrupt();
#endif
	return 0;
}

/**
 * ax88796_close - shut down network device
 * @dev: network device to close
 *
 * Opposite of ax88796_open(). Only used when "ifconfig <devname> down" is done.
 */
int ax88796_close(struct net_device *dev)
{
	struct ei_device *ei_local = (struct ei_device *) dev->priv;
	unsigned long flags;
...

/**
 * ax88796_interrupt - handle the interrupts from an 8390
...
#if defined(CONFIG_ARCH_DBMX2)
	if((_reg_GPIO_ISR(GPIOE) & (0x1<<11)) == 0)
		return;		// another shared irq
	Clear_GPIO_E_11_interrupt();
#endif
...
int ax88796_ethdev_init(struct net_device *dev)
{
	if (ei_debug > 1)
		printk(version);
...
/**
 * AX88796_init - initialize AX88796 hardware
 * @dev: network device to initialize
...
void AX88796_init(struct net_device *dev, int startp)
{
	long e8390_base = dev->base_addr;
	struct ei_device *ei_local = (struct ei_device *) dev->priv;
	int i;
...
EXPORT_SYMBOL(ax88796_open);
EXPORT_SYMBOL(ax88796_close);
EXPORT_SYMBOL(ax88796_interrupt);
EXPORT_SYMBOL(ax88796_ethdev_init);
EXPORT_SYMBOL(AX88796_init);
