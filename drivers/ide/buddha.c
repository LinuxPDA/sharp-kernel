...
#define XSURF_CONTROL   -1              /* X-Surf has no CS1* */
					/* (Control/AltStat) */
...
static int xsurf_ack_intr(ide_hwif_t *hwif)
{
    ...
    ch = z_readb(hwif->io_ports[IDE_IRQ_OFFSET]);
    /* X-Surf needs a 0 written to IRQ register to ensure ISA bit A11 stays */
    /* at 0 */
...
fail_base2:
	...
	for (i=0;i<buddha_num_hwifs;i++) {
	    if (type != BOARD_XSURF) {
		ide_setup_ports(&hw,
				(ide_ioreg_t)(buddha_board+buddha_bases[i]),
				buddha_offsets, 0,
				(ide_ioreg_t)(buddha_board+buddha_irqports[i]),
				buddha_ack_intr, IRQ_AMIGA_PORTS);
	    } else {
		ide_setup_ports(&hw,
				(ide_ioreg_t)(buddha_board+xsurf_bases[i]),
				xsurf_offsets, 0,
