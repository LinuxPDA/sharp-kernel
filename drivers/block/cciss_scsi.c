...
scsi_cmd_alloc(ctlr_info_t *h)
{
	...
	c->cmd.busaddr = (__u32) (stk->cmd_pool_handle + 
		(sizeof(struct cciss_scsi_cmd_stack_elem_t)*stk->top));

	temp64.val = (__u64) (stk->cmd_pool_handle + 
		(sizeof(struct cciss_scsi_cmd_stack_elem_t)*stk->top) +
...
scsi_cmd_stack_free(int ctlr)
{
	...
	pci_free_consistent(hba[ctlr]->pdev, size, stk->pool, 
			stk->cmd_pool_handle);
	stk->pool = NULL;
}
