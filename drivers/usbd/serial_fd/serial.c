...
static __inline__ struct usb_serial_private *
get_serial_private(int interface)
{
 ...
 */
static __initdata struct usb_alternate_description serial_data_alternate_descriptions[] = {
    {   iInterface: "Simple Serial Data Interface - Bulk mode", 
        bAlternateSetting: 0,
        endpoints: sizeof(serial_default)/sizeof(struct usb_endpoint_description),
...
static struct usb_class_description cdc_comm_class_descriptions[] = {
    ...
    { USB_ST_CMF,     0, { call_management: { bmCapabilities: 0, bDataInterface: 1, }}},
    { USB_ST_ACMF,    0, { abstract_control: { bmCapabilities: 0, }}},
};


/* Data Interface Alternate 1 endpoints
 */
static __initdata struct usb_endpoint_description serial_alt_1_endpoints[] = {
    { bEndpointAddress: CONFIG_USBD_SERIAL_OUT_ENDPOINT, 
        bmAttributes: BULK, 
        wMaxPacketSize: CONFIG_USBD_SERIAL_OUT_PKTSIZE, 
        bInterval: 0, 
...
static __initdata struct usb_alternate_description cdc_data_alternate_descriptions[] = {
    ...
    {   iInterface: "CDC ACM Data Interface - Bulk mode", 
        bAlternateSetting: 1,
        endpoints: sizeof(serial_alt_1_endpoints)/sizeof(struct usb_endpoint_description),
...
static int serial_xmit_data(int interface, unsigned char *data, int data_length)
{
    ...
    if ((urb = usbd_alloc_urb(serial_private->device, 
                    (serial_private->device->function_instance_array+port), CONFIG_USBD_SERIAL_IN_ENDPOINT, 0))==NULL) 
    {
    ...
    if (safe) {
        ...
        dbg_tx(1, "safe mode: padded: %d data_length: %d packet_length: %d", padded, data_length, packet_length);
        ...
        packet_length += 2;
        dbg_tx(1, "safe mode: data_length: %d packet_length: %d", data_length, packet_length);
    }
#endif
...
int serial_urb_sent (struct urb *urb)
{
    ...
    struct usb_serial_private *serial_private = (device->function_instance_array+port)->privdata;

    dbg_tx(2,"%s safe: %d length: %d",urb->device->name, safe, urb->actual_length);
    serproto_done(serial_private->interface, 
            urb->buffer, safe ? (urb->buffer[urb->actual_length - 2] >> 2): urb->actual_length, 0);  
...
void serial_event(struct usb_device_instance *device, usb_device_event_t event, int data)
{
    ...
    case DEVICE_CREATE:
        dbg_usbe(1,"---> CREATE %s %d", device->name, event);
        {
            ...
            if (serial_created >= MAX_INTERFACES) {
                dbg_usbe(1,"---> CREATE %s serial_created >= MAX_INTERFACES %d %d", 
                         device->name, serial_created, MAX_INTERFACES);
                dbg_init(0,"CREATE Z1 sc=%d uc=%d",serial_created,GET_USE_COUNT(THIS_MODULE));
                return;
            }
            ...
            if (i >= MAX_INTERFACES) {
                ...
                dbg_init(0,"CREATE Z2 sc=%d uc=%d",serial_created,GET_USE_COUNT(THIS_MODULE));
                return;
            }
            serial_created++;

            // allocate private data
            if ((serial_private = kmalloc(sizeof(struct usb_serial_private),GFP_ATOMIC))==NULL) {
                serial_created--;
                //MOD_DEC_USE_COUNT;
            ...
            dbg_usbe(1,"---> calling serproto_create(%s,-,%u,%u,%u,%u,%u)","ttyUSBx", 
                    CONFIG_USBD_SERIAL_IN_PKTSIZE, txqueue_urbs, txqueue_bytes, safe, safe?2:0);

            if ((interface = serproto_create("ttyUSBx", serial_xmit_data, 
                            (safe ? (CONFIG_USBD_SERIAL_IN_PKTSIZE - 2) : CONFIG_USBD_SERIAL_IN_PKTSIZE), 
                            txqueue_urbs, txqueue_bytes, safe, safe ? 2 : 0)) < 0) 
            {
            ...
            dbg_usbe(1,"---> START %s privdata assigned: %p interface: %d", device->name, serial_private, interface); 
    ...
    case DEVICE_CONFIGURED:
        dbg_usbe(1,"---> CONFIGURED %s %d", device->name, event);
        {
            struct usb_serial_private *serial_private;
            int interface;
            if ((serial_private = (device->function_instance_array+port)->privdata) == NULL) {
                return;
            }
    ...
    case DEVICE_DESTROY:
        dbg_usbe(1, "---> DESTROY %s %d", device->name, event);
        {
            ...
            dbg_usbe(1, "---> DESTROY %s serial_private %p", device->name, serial_private);
            interface = serial_private->interface;
