...
static void sl11_dump_epn(unsigned char ep, char *m)
{
    printk("ep[%d]: Ctl:%02x Adr:%02x Xfr:%02x Sts:%02x Cnt:%02x Seq:%d%s", ep,
            ...
            sl11read_epn(ep, EPN_Counter),
            ep_sequence[ep&0x3], m);
}
...
static int sl11_probe(void)
{
    ...
    if ( ((sl11read_byte(IntStatus) & INT_USB_RESET) != INT_USB_RESET) || ((sl11read_byte(DATASet) & 0x10) != 0x10)) {
        sl11_dump(0, 64);
    ...
    if ((sl11read_byte(IntStatus) != 0x00)) {
        sl11_dump(0, 64);
        printk(KERN_DEBUG"sl11_probe: cannot see " UDC_NAME ", failed interrupt status reset\n");
        udc_regs();
        return -EINVAL;
    }
...
static void __inline__ sl11send_done(unsigned ep) 
{
    // disarm
    sl11write_epn(ep, EPN_Control, 
            EPN_CTRL_ENABLE | EPN_CTRL_DIRECTION_IN) ;
...
static void __inline__ sl11_out(unsigned int ep, struct usb_endpoint_instance *endpoint)
{
    ...
    for(i = 0; i < 8; i++) {
        ...
        if (endpoint->rcv_urb && len) {
            sl11read_buffer( adr, endpoint->rcv_urb->buffer + endpoint->rcv_urb->actual_length, len);
        }
 ...
 * @ep:
 */
static void __inline__ sl11_start(unsigned int ep, struct usb_endpoint_instance *endpoint, int restart)
{
...
void udc_reset_ep(unsigned int ep)
{
    ...
    if (!ep) {
        ...
        // arm EP0
        sl11write_epn(0, EPN_Control, EPN_CTRL_ARM | EPN_CTRL_ENABLE | EPN_CTRL_DIRECTION_OUT );
...
void udc_setup_ep(struct usb_device_instance * device, unsigned int ep, struct usb_endpoint_instance *endpoint)
{
    if (ep < UDC_MAX_ENDPOINTS) {
        ...
        // IN
            ...
            sl11write_epn(ep, EPN_Address, EP_S_BUF);
            sl11write_epn(ep, EPN_Control, EPN_CTRL_ARM | EPN_CTRL_ENABLE | EPN_CTRL_DIRECTION_IN );
        }
        // OUT
            ...
            sl11write_epn(ep + 0, EPN_Control, EPN_CTRL_ARM | EPN_CTRL_ENABLE | EPN_CTRL_DIRECTION_OUT );
            sl11write_epn(ep + 0, EPN_Control, EPN_CTRL_ARM | EPN_CTRL_ENABLE | EPN_CTRL_DIRECTION_OUT | EPN_CTRL_NEXTDATA );

            sl11write_epn(ep + 4, EPN_Control, EPN_CTRL_ARM | EPN_CTRL_ENABLE | EPN_CTRL_DIRECTION_OUT );
...
int udc_request_udc_irq()
{
    ...
    if (request_irq(UDC_IRQ, sl11_int_hndlr, SA_INTERRUPT, UDC_NAME, NULL) != 0) {
