 ...
 */
static __inline__ void usbd_rcv_complete_irq(struct usb_endpoint_instance *endpoint, int len, int status)
{
    ...
    if (endpoint) {
        ...
        if ((rcv_urb = endpoint->rcv_urb)) {
            ...
            if ((rcv_urb->device->status == USBD_OK) && (len <= endpoint->rcv_packetSize)) {
                ...
                if (!status && ((len < endpoint->rcv_packetSize) || 
		...
		printk(KERN_ERR"usbd_rcv_complete: RECV_ERROR actual: %d buffer: %d status: %d\n", 
			rcv_urb->actual_length, rcv_urb->buffer_length, status);
...
static __inline__ void usbd_tx_complete_irq(struct usb_endpoint_instance *endpoint, int restart)
{
    if (endpoint) {
        ...
        if (endpoint->endpoint_address && !endpoint->tx_urb && (endpoint->tx_urb = first_urb_detached_irq(&endpoint->tx))) {
            endpoint->last = endpoint->sent = 0;
        }
    }
    else {
