...
USBD_MODULE_INFO("net_fd 0.1-early");
...
#ifdef CONFIG_USBD_NET_CDC
MODULE_PARM_DESC(remote_mac_address, "MAC");
#endif
MODULE_PARM_DESC(local_mac_address, "s");
MODULE_PARM_DESC(vendor_id, "i");
MODULE_PARM_DESC(product_id, "i");
MODULE_PARM_DESC(alwaysup, "i");
MODULE_PARM_DESC(dbg, "s");
...
#error One of CONFIG_USBD_NET_SAFE or CONFIG_USBD_NET_CDC must be set
#endif
...
static __initdata struct usb_endpoint_description net_default[] = {
 ...
 */
static __initdata struct usb_alternate_description net_data_alternate_descriptions[] = {
    {   iInterface: "Simple Ethernet Data Interface - Bulk mode", 
        bAlternateSetting: 0,
 ...
 */
static __initdata struct usb_interface_description net_interfaces[] = {
    {   iInterface: "Simple Ethernet Data Interface", 
 ...
 */
static __initdata struct usb_endpoint_description net_alt_1_endpoints[] = {
    { bEndpointAddress: CONFIG_USBD_NET_OUT_ENDPOINT, 
        bmAttributes: BULK, 
        wMaxPacketSize: CONFIG_USBD_NET_OUT_PKTSIZE, 
        bInterval: 0, 
...
/* Data Interface Alternate description(s)
 */
static __initdata struct usb_alternate_description cdc_comm_alternate_descriptions[] = {
    {   iInterface: "CDC Ethernet Comm Interface", 
        bAlternateSetting: 0, 
        classes: sizeof(cdc_comm_class_descriptions)/sizeof(struct usb_class_description),
        class_list: cdc_comm_class_descriptions, },
};

static __initdata struct usb_alternate_description cdc_data_alternate_descriptions[] = {
    {   iInterface: "CDC Ethernet Data Interface - Disabled mode", 
        bAlternateSetting: 0, },

    {   iInterface: "CDC Ethernet Data Interface - Bulk mode", 
        bAlternateSetting: 1,
...
/* Interface description(s)
 */
static __initdata struct usb_interface_description cdc_interfaces[] = {
    {   iInterface: "CDC Ethernet Communication Interface", 
    ...
    {   iInterface: "CDC Ethernet Data Interface", 
 ...
 */
struct __initdata usb_configuration_description net_description[] = {
#ifdef CONFIG_USBD_NET_CDC
...
/* Device Description
 */
struct __initdata usb_device_description net_device_description = {
...
{
    printk(KERN_DEBUG "net_dev_kfree_skb: skb: %p head: %p data: %p tail: %p end: %p len: %d\n", 
...
void net_create(struct usb_device_instance *device, struct usb_function_instance *function)
{
    ...
    MOD_INC_USE_COUNT;
    dbg_init(7,"MOD_INC_USE_COUNT %d", GET_USE_COUNT(THIS_MODULE));
    ...
    if ((net_private->interface = netproto_create(net_private->name, net_xmit_skb, net_set_addr, net_tx_timeout,
                    default_dev_addr, ETH_ALEN, 1500, 1, 4000, 0 )) < 0)
...
void net_destroy(struct usb_device_instance *device)
{
    ...
    MOD_DEC_USE_COUNT;
    dbg_init(7,"MOD_DEC_USE_COUNT %d", GET_USE_COUNT(THIS_MODULE));
    return;
}
        ...
        struct usb_function_driver *function_driver)
{
    ...
    if (strlen(remote_mac_address_buffer) != 12) {
        dbg_init(0, "mac_address has wrong length: %s %d", remote_mac_address_buffer, strlen(remote_mac_address_buffer));
        //return -EINVAL;
        return;
    }
    ...
    for (i = 0; i < sizeof(cdc_comm_class_descriptions)/sizeof(struct usb_class_description); i++) {
        ...
        if (class_description->bDescriptorSubtype == USB_ST_ENF) {
            class_description->description.ethernet_networking.iMACAddress = remote_mac_address_buffer;
            break;
        }
    }
#endif

    // set dev addr if serial
#if 0
    if (local_mac_address && (strlen(local_mac_address) == 12)) {
        ...
        dbg_init(1, "using serial:  %d %d", CONFIG_USBD_NET_LOCAL_OUI, serial);
        sprintf(local_mac_address_buffer, "%06X%06X", CONFIG_USBD_NET_LOCAL_OUI, serial);
    }
#endif

    dbg_init(1, "-------------------------------------------------------------------------------"); 
    // XXX XXX net_create(device, function);
    //net_private->device = device;
    //function->privdata = net_private;
...
void net_event(struct usb_device_instance *device, usb_device_event_t event, int data)
{
    ...
    dbg_usbe(1, "--- event: %d data: %d", event, data);
    ...
    case DEVICE_FUNCTION_PRIVATE:
        dbg_usbe(3, "DEVICE_FUNCTION_PRIVATE");
        dbg_usbe(3, "DEVICE_FUNCTION_PRIVATE: data: %d", data);
        switch (data) {
        case NET_SET_CRC:
            break;
        }
...
int net_recv_setup (struct urb *urb)
{
    ...
    if ((request->bmRequestType&USB_REQ_TYPE_MASK)!=USB_REQ_TYPE_VENDOR) {
        dbg_ep0(1, "not vendor request: %x %x", request->bmRequestType, request->bmRequestType&USB_REQ_TYPE_MASK);
        return 0; // XXX
    }
...
int net_recv_urb (struct urb *urb)
{
    ...
    dbg_rx(5, "urb: %p len: %d maxtransfer: %d\n", urb, urb->actual_length, net_private->maxtransfer);

    length = urb->actual_length;

    if (length > net_private->maxtransfer) {
        dbg_rx(0, "TOO LARGE urb->actual_length: %d maxtransfer: %d", urb->actual_length, net_private->maxtransfer);
        netproto_rx_dropped(device->interface);
        return -EINVAL;
    }
    ...
    if (net_private->crc) {
        ...
        if ((length % out_pkt_sz) == 1) {
            ...
            if (((fcs = fcs_memcpy32(skb_put(skb, length - 1), urb->buffer, length - 1, CRC32_INITFCS)) != CRC32_GOODFCS)) {

                // CRC32 check failed, fcs and copy last byte, if ok then continue otherwise fail
                if ((fcs_memcpy32(skb_put(skb, 1), urb->buffer + length - 1, 1, fcs) != CRC32_GOODFCS)) {
                    dbg_rx(0,"CRC32 failed on extra byte len: %d fcs: %08x", length, fcs);
                    dev_kfree_skb_any(skb);
                    netproto_rx_errors(device->interface);
                    return -EINVAL;
                }
            }
        }
        else {
            if (((fcs = fcs_memcpy32(skb_put(skb, length), urb->buffer, length, CRC32_INITFCS)) != CRC32_GOODFCS)) {
                dbg_rx(0,"CRC32 failed len: %d fcs: %08x", length, fcs);
                dev_kfree_skb_any(skb);
                netproto_rx_errors(device->interface);
                return -EINVAL;
            }
...
static int net_xmit_skb(int interface, struct sk_buff *skb)
{
    ...
    if (net_private->first) {
        ...
        for (i = 0; i < 3; i++) {
            if ((urb = usbd_alloc_urb(net_private->device, (net_private->device->function_instance_array+port), 
                            CONFIG_USBD_NET_IN_ENDPOINT, 200))) 
            {
    ...
    if (!(urb = usbd_alloc_urb(net_private->device, (net_private->device->function_instance_array+port), 
                    CONFIG_USBD_NET_IN_ENDPOINT, skb->len + 5 + in_pkt_sz))) 
    {
    ...
    if (net_private->crc) {
        ...
        // if transfer is less than packetsize guarantee that is at lest two packets
            ...
            dbg_tx(2, "do_fcs32: len: %d padding; %d", skb->len, (in_pkt_sz - skb->len - 3));
            send_fcs = fcs_pad32(urb->buffer + urb->actual_length, (in_pkt_sz - skb->len - 3), send_fcs);
...
static int __init net_modinit(void)
{
    ...
    if (local_mac_address) {
        dbg_init(1, "localmac_address: %s len: %d", local_mac_address, strlen(local_mac_address));
    }

    // save the mac address for the network device
    for (i = 0; i < ETH_ALEN; i++) {
        default_dev_addr[i] = hexdigit(local_mac_address_buffer[i*2])<<4 | hexdigit(local_mac_address_buffer[i*2+1]);
        //dbg_init(2, "local_mac_address[%d]: %02x %02x %d %d %02x", i, 
