...
netproto_get_stats(struct net_device *net_dev)
{
    struct netproto_dev *device = net_dev->priv;
    printk(KERN_DEBUG"get_stats rx_dropped: %d\n", device->stats.rx_dropped);
    printk(KERN_DEBUG"get_stats rx_errors: %d\n", device->stats.rx_errors);
    return &device->stats;
}
 ...
 */
static void
netproto_set_multicast(struct net_device *net_dev)
{
    //struct netproto_dev *device = net_dev->priv;
 ...
 */
static int
netproto_set_mac_addr(struct net_device *net_dev, void *p)
{
    struct netproto_dev *device = net_dev->priv;
 ...
 */
static void
netproto_tx_timeout(struct net_device *net_dev)
{
    struct netproto_dev *device = net_dev->priv;
 ...
 */
static int
netproto_set_config (struct net_device *net_dev, struct ifmap *map)
{
    if (netif_running(net_dev)) {
        return -EBUSY;
    }
 ...
 */
static int
netproto_change_mtu (struct net_device *net_dev, int mtu)
{
    struct netproto_dev *dev = net_dev->priv;
 ...
 */
static int
netproto_start_xmit(struct sk_buff *skb, struct net_device *net_dev)
{
    struct netproto_dev *device = net_dev->priv;
    int len = skb->len;
    int rc;
    ...
    *(time_t *)(&skb->cb) = jiffies;
    ...
    if ((rc = device->xmit_skb(device->interface, skb))) {
...

        {
            ...
            if ((device->queued_entries < device->max_queue_entries) && (device->queued_bytes < device->max_queue_bytes)) {
 ...
 */
static int
netproto_do_ioctl(struct net_device *net_dev, struct ifreq *rp, int cmd)
{
    //int             rc;
 ...
 */
static ssize_t
netproto_proc_read(struct file *file, char *buf, size_t count, loff_t * pos)
{
    unsigned long   page;
    int len = 0;
    int index;
    ...
    for (i=0; i<netproto_devices; i++) {
        ...
        if (index-- == 0) {
            ...
            len += sprintf((char *)page+len, "%.16s %2d [%02X:%02X:%02X:%02X:%02X:%02X]",
                    dev->net_dev->name, dev->interface,
            ...
            len += sprintf((char *)page+len, " [%6d %2d]", dev->stopped, dev->stopped-dev->restarts);
            len += sprintf((char *)page+len, " [%4ld]", 
                    dev->samples?((dev->jiffies*1000)/dev->samples):0);

            len += sprintf((char *)page+len, " [%3ld]", dev->samples?((dev->avg_queue_entries)/dev->samples):0);
...
*
 */
void netproto_rx_dropped(struct netproto_dev *device)
{
    if (device) {
        unsigned long flags;
        write_lock_irqsave(&device->rwlock, flags);
    ...
    printk(KERN_DEBUG"rx_dropped: %d\n", device->stats.rx_dropped);
}
...
*
 */
void netproto_rx_errors(struct netproto_dev *device)
{
    if (device) {
        unsigned long flags;
        write_lock_irqsave(&device->rwlock, flags);
...
*
 */
void netproto_rx_packets(struct netproto_dev *device, int len)
{
    if (device) {
        unsigned long flags;
        write_lock_irqsave(&device->rwlock, flags);
...
*
 */
void netproto_tx_dropped(struct netproto_dev *device)
{
    if (device) {
        unsigned long flags;
        write_lock_irqsave(&device->rwlock, flags);
...
*
 */
void netproto_tx_carrier_errors(struct netproto_dev *device)
{
    if (device) {
        unsigned long flags;
        write_lock_irqsave(&device->rwlock, flags);
...
*
 */
void netproto_tx_fifo_errors(struct netproto_dev *device)
{
    if (device) {
        unsigned long flags;
        write_lock_irqsave(&device->rwlock, flags);
...
*
 */
void netproto_tx_packets(struct netproto_dev *device, int len)
{
    if (device) {
        unsigned long flags;
        write_lock_irqsave(&device->rwlock, flags);
...
*
 */
char * netproto_name(struct netproto_dev *device)
{
    if (device) {
...
int netproto_modinit(char *name, int maximum_interfaces)
{
    ...
    if ((nda = kmalloc(sizeof(struct netproto_dev *) * maximum_interfaces, GFP_ATOMIC))==NULL) {
        ...
        int flags)
{
    ...
    printk(KERN_DEBUG"create rx_dropped: %d\n", device->stats.rx_dropped);
    printk(KERN_DEBUG"create rx_errors: %d\n", device->stats.rx_errors);
