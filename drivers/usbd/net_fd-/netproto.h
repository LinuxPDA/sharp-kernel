 ...
 *
 */
void netproto_rx_dropped(struct netproto_dev *);
 ...
 *
 */
void netproto_rx_errors(struct netproto_dev *);
 ...
 *
 */
void netproto_rx_packets(struct netproto_dev *, int);
 ...
 *
 */
void netproto_tx_dropped(struct netproto_dev *);
 ...
 *
 */
void netproto_tx_carrier_errors(struct netproto_dev *);
 ...
 *
 */
void netproto_tx_fifo_errors(struct netproto_dev *);
 ...
 *
 */
void netproto_tx_packets(struct netproto_dev *, int);
 ...
 *
 */
char * netproto_name(struct netproto_dev * interface);
...
static __inline__ int netproto_recv(int interface, struct sk_buff *skb)
{
...
#if 0
    dbg_prx(7, "skb: %p head: %p data: %p tail: %p end: %p len: %d", 
            skb, skb->head, skb->data, skb->tail, skb->end, skb->len);
    if (dbgflg_usbdfd_prx >= 3) {
        int i;
        if ((i = fcs_compute(skb->data, skb->len+2, CRC_INITFCS)) != CRC_GOODFCS) {
            dbg_prx(1,"skb has BAD FCS : %x", i);
        }
        dbgPRINTmem(dbgflg_usbdfd_prx,7,skb->data,skb->len+2);
    }
#endif

    //dbg_prx(0, "interface: %d", interface);
    //dbg_prx(0, "netproto_device_array: %p", netproto_device_array);
    //dbg_prx(0, "device = netproto_device_array[interface]: %p", netproto_device_array[interface]);
    ...
    if (!(net_dev->flags & IFF_UP)) {
        dbg_prx(0, "not up net_dev.state: %x", net_dev->flags);
    ...
    dbg_prx(5, "skb: %p head: %p data: %p tail: %p end: %p len: %d", skb, skb->head, skb->data, skb->tail, skb->end, skb->len);
