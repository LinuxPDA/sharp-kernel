...
void usbd_device_event_irq(struct usb_device_instance *device, usb_device_event_t event, int data)
{
    ...
    switch(event) {
    case DEVICE_UNKNOWN:
        dbg_usbe(1, "DEVICE_UNKNOWN");
        break;
    case DEVICE_INIT:
        dbg_usbe(3,"DEVICE_INIT");
    ...
    case DEVICE_RESET:
        dbg_usbe(1,"DEVICE_RESET, setting address to zero");
        device->device_state = STATE_DEFAULT;
        device->address = 0;
        break;

    case DEVICE_ADDRESS_ASSIGNED:
        dbg_usbe(1,"DEVICE_ADDRESS_ASSIGNED");
        device->device_state = STATE_ADDRESSED;
        break;

    case DEVICE_CONFIGURED:
        dbg_usbe(1, "DEVICE_CONFIGURED");
        device->device_state = STATE_CONFIGURED;
        break;

    case DEVICE_DE_CONFIGURED:
        dbg_usbe(1, "DEVICE_DE_CONFIGURED");
        device->device_state = STATE_ADDRESSED;
        break;

    case DEVICE_BUS_INACTIVE:
        dbg_usbe(1, "DEVICE_INACTIVE");
        if (device->status != USBD_CLOSING) {
            device->status = USBD_SUSPENDED;
        }
        break;
    case DEVICE_BUS_ACTIVITY:
        dbg_usbe(1, "DEVICE_ACTIVITY");
        if (device->status != USBD_CLOSING) {
    ...
    case DEVICE_SET_INTERFACE:
        dbg_usbe(1, "DEVICE_SET_INTERFACE");
        break;
    case DEVICE_SET_FEATURE:
        dbg_usbe(1, "DEVICE_SET_FEATURE");
        break;
    case DEVICE_CLEAR_FEATURE:
        dbg_usbe(1, "DEVICE_CLEAR_FEATURE");
        break;

    case DEVICE_POWER_INTERRUPTION:
        dbg_usbe(1, "DEVICE_POWER_INTERRRUPTION");
        device->device_state = STATE_POWERED;
        break;
    case DEVICE_HUB_RESET:
        dbg_usbe(1, "DEVICE_HUB_RESET");
        device->device_state = STATE_ATTACHED;
        break;
    case DEVICE_DESTROY:
        dbg_usbe(1, "DEVICE_DESTROY");
        device->device_state = STATE_UNKNOWN;
        break;
    case DEVICE_FUNCTION_PRIVATE:
        dbg_usbe(1, "DEVICE_FUNCTION_PRIVATE");
        break;
    }
