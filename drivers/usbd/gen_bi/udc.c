 ...
 * @ep:
 */
static void __inline__ start_in(unsigned int ep, struct usb_endpoint_instance *endpoint, int restart)
{
    if (endpoint->tx_urb) {
...
int udc_request_udc_irq()
{
    ...
    if (request_irq(XXXX_UDC_IRQ, int_hndlr, SA_INTERRUPT, UDC_NAME " USBD Bus Interface", NULL) != 0) {
...
int udc_request_cable_irq()
{
    ...
    // request IRQ  and IO region
    if (request_irq(XXXX_CABLE_IRQ, int_hndlr, SA_INTERRUPT, UDC_NAME " CAble", NULL) != 0) {
        printk(KERN_DEBUG"usb_ctl: Couldn't request USB irq\n");
        return -EINVAL;
    }
#endif
