/*
 * linux/drivers/usbd/cdsp_bi/cdsp_init.c - USB device CDSP bus interface
 *
...
static debug_option dbg_table[] = {
    {&dbg_init,NULL,"init","initialization and termination"},
    {&dbg_intr,NULL,"intr","interrupt handling"},
    {&dbg_tick,NULL,"tick","interrupt status monitoring on clock tick"},
    {&dbg_usbe,NULL,"usbe","USB events"},
    {&dbg_rstr,NULL,"rstr","USB RSTR interrupt handling"},
    {&dbg_setup,NULL,"setup","Setup packet handling"},
    {NULL,NULL,NULL,NULL}
};
...
void cdsp_connect(struct usb_bus_instance *bus)
{
    ...
    if (dbg_init >= 1) {
        ...
        dbgPRINT(&dbg_init,1,"USB_Control#%02x->#%02x(-#%02x+#%02x)",
...
void cdsp_disconnect(struct usb_bus_instance *bus)
{
    ...
    if (dbg_init >= 1) {
        nval = reg32read(data->mcu_base+USB_Control);
        dbgPRINT(&dbg_init,1,"USB_Control#%02x->#%02x(-#%02x)",
                 oval,nval,(UC_Rmt_Wkup|UC_Connect));
    }
}
        ...
        int allow_resume)
{
    ...
    if (dbg_intr >= 1 /* && repeat_count <= 2 */) {
        nval = reg32read(bd->mcu_base+USB_Interrupt_Mask);
        dbgPRINT(&dbg_intr,1,"UI_Mask#%02x->#%02x(-#%02x+#%02x)",
                 oval,nval,dval,cval);
    }
}
...
void cdsp_enable_usb_reset(struct usb_bus_instance *bus)
{
    ...
    if (dbg_rstr >= 1) {
        nval = reg32read(data->mcu_base+USB_Control);
        dbgPRINT(&dbg_rstr,1,"USB_Control#%02x->#%02x(+#%02x)",
...
int cdsp_do_reset(bi_data *bd)
{
    ...
    if (5 <= dbg_rstr) {
        ...
        dbgPRINT(&dbg_rstr,5,"RSR#%08x",oval);
    }
    ...
    dbgPRINT(&dbg_rstr,3,"RESET PRCR#%08x->#%08x",oval,nval);
...
{
    dbgPRINT(&dbg_init,1,"endpoint %d flag %d",endpoint,flag);
    if (flag) {
...
int cdsp_device_event(struct usb_device_instance *device, usb_device_event_t event) 
{
    ...
    switch (event) {
    case DEVICE_CREATE:
        dbgPRINT(&dbg_usbe,1,"CREATE");
        // enable upstream port
        cdsp_enable_usb_interrupts(device->bus,rstie);
        cdsp_connect(device->bus);
        break;
    case DEVICE_DESTROY:
        dbgPRINT(&dbg_usbe,1,"DESTROY");
        // disable upstream port
        cdsp_disconnect(device->bus);
        cdsp_disable_usb_interrupts(device->bus,0);
        break;
    case DEVICE_HUB_CONFIGURED:
        dbgPRINT(&dbg_usbe,1,"HUB_CONFIGURED");
        break;
    case DEVICE_HUB_RESET:
        dbgPRINT(&dbg_usbe,1,"HUB_RESET");
        break;
    case DEVICE_RESET:
        dbgPRINT(&dbg_usbe,1,"DEVICE_RESET");
        break;
    case DEVICE_POWER_INTERRUPTION:
        dbgPRINT(&dbg_usbe,1,"DEVICE_POWER_INTERRUPTION");
        break;

    case DEVICE_ADDRESS_ASSIGNED:
        dbgPRINT(&dbg_usbe,1,"ADDRESS");
        // udc_set_address(device->address);
        break;

    case DEVICE_CONFIGURED:
        dbgPRINT(&dbg_usbe,1,"DEVICE_CONFIGURED");
        break;
    case DEVICE_DE_CONFIGURED:
        dbgPRINT(&dbg_usbe,1,"DEVICE_DE_CONFIGURED");
        break;

    case DEVICE_BUS_INACTIVE:
        dbgPRINT(&dbg_usbe,1,"DEVICE_BUS_INACTIVE");
        cdsp_disable_usb_interrupts(device->bus,1);
        // udc_disable_interrupts(1);  1 ===> suspend interr only? QQQ
        break;

    case DEVICE_BUS_ACTIVITY:
        dbgPRINT(&dbg_usbe,1,"DEVICE_BUS_ACTIVITY");
        cdsp_enable_usb_interrupts(device->bus,rstie);
        // udc_enable_interrupts(1);  1 ===> suspend interr only? QQQ
        break;

    case DEVICE_SET_INTERFACE:
        dbgPRINT(&dbg_usbe,1,"DEVICE_SET_INTERFACE");
        break;
    case DEVICE_SET_FEATURE:
        dbgPRINT(&dbg_usbe,1,"DEVICE_SET_FEATURE");
        break;
    case DEVICE_CLEAR_FEATURE:
        dbgPRINT(&dbg_usbe,1,"DEVICE_CLEAR_FEATURE");
        break;
    default:
        dbgPRINT(&dbg_init,1,"unknown event %d",event);
    }
...
static void cdsp_interrupt(int x, void *vp, struct pt_regs *regs)
{
    ...
    // dbgPRINT(&dbg_init,1,"x=%d vp#%08x",x,(u32)vp);
    // /* Read the Avalanche interrupt status register. */
    // val = reg32read(AV_INTR_BASE+AVI_INTSR1);
    // dbgPRINT(&dbg_init,1,"x=%d vp#%08x val#%08x",x,(u32)vp,val);
    ...
    if (prev_USB_status != ost) {
        ...
        case US_RSTR:
            ...
            repeating = 1;
            dbgPRINT(&dbg_intr,1,"repeating RSTR => UI_Mask#%02x",
        ...
        case US_SUSR:
            ...
            repeating = 1;
            dbgPRINT(&dbg_intr,1,"repeating SUSR => UI_Mask#%02x",
                     reg32read(bd->mcu_base+USB_Interrupt_Mask));
            break;
        case US_RESR:
            ...
            repeating = 1;
            dbgPRINT(&dbg_intr,1,"repeating RESR => UI_Mask#%02x",
                     reg32read(bd->mcu_base+USB_Interrupt_Mask));
            break;
    ...
    do {
        ...
        if (bailing_out) {
            ...
            if (val != oval) {
                ...
                case VI_STPOW:
                    ...
                    dbgPRINT(&dbg_intr,1,"stuck STPOW => UI_Mask#%02x",
                             reg32read(bd->mcu_base+USB_Interrupt_Mask));
                    break;
                case VI_SETUP:
                    ...
                    dbgPRINT(&dbg_intr,1,"stuck SETUP => UI_Mask#%02x",
                             reg32read(bd->mcu_base+USB_Interrupt_Mask));
                    break;
                case VI_RESR:
                    //uim = reg32read(bd->mcu_base+USB_Interrupt_Mask) & ~UI_RESR_Enable;
                    ...
                    bd->desired_ui_mask = uim;
                    stuck_disabled |= UI_RESR_Enable;
                    dbgPRINT(&dbg_intr,1,"stuck RESR => UI_Mask#%02x",
                             reg32read(bd->mcu_base+USB_Interrupt_Mask));
                    break;
                case VI_SUSR:
                    //uim = reg32read(bd->mcu_base+USB_Interrupt_Mask) & ~UI_SUSR_Enable;
                    ...
                    bd->desired_ui_mask = uim;
                    stuck_disabled |= UI_SUSR_Enable;
                    dbgPRINT(&dbg_intr,1,"stuck SUSR => UI_Mask#%02x",
                             reg32read(bd->mcu_base+USB_Interrupt_Mask));
                    break;
                case VI_RSTR:
                    //uim = reg32read(bd->mcu_base+USB_Interrupt_Mask) & ~UI_RSTR_Enable;
                    ...
                    bd->desired_ui_mask = uim;
                    stuck_disabled |= UI_RSTR_Enable;
                    dbgPRINT(&dbg_intr,1,"stuck RSTR => UI_Mask#%02x",
                             reg32read(bd->mcu_base+USB_Interrupt_Mask));
                    break;
                default:
                    // Yet another kinda stuck interrupt?
                    dbgPRINT(&dbg_intr,1,"Stuck on #%02x!",val);
                }
        ...
        switch (val) {
        case VI_OE1:
            dbgPRINT(&dbg_intr,5,"OE1");
            break;
        case VI_OE2:
            dbgPRINT(&dbg_intr,5,"OE2");
            break;
        case VI_OE3:
            dbgPRINT(&dbg_intr,5,"OE3");
            break;
        case VI_OE4:
            dbgPRINT(&dbg_intr,5,"OE4");
            break;
        case VI_OE5:
            dbgPRINT(&dbg_intr,5,"OE5");
            break;
        case VI_OE6:
            dbgPRINT(&dbg_intr,5,"OE6");
            break;
        case VI_OE7:
            dbgPRINT(&dbg_intr,5,"OE7");
            break;
        case VI_IE1:
            dbgPRINT(&dbg_intr,5,"IE1");
            break;
        case VI_IE2:
            dbgPRINT(&dbg_intr,5,"IE2");
            break;
        case VI_IE3:
            dbgPRINT(&dbg_intr,5,"IE3");
            break;
        case VI_IE4:
            dbgPRINT(&dbg_intr,5,"IE4");
            break;
        case VI_IE5:
            dbgPRINT(&dbg_intr,5,"IE5");
            break;
        case VI_IE6:
            dbgPRINT(&dbg_intr,5,"IE6");
            break;
        case VI_IE7:
            dbgPRINT(&dbg_intr,5,"IE7");
            break;
        case VI_STPOW:
            dbgPRINT(&dbg_intr,5,"STPOW");
            /* Clear status bit. */
        ...
        case VI_RSTR:
            ...
            break;
        case VI_IE0:
            dbgPRINT(&dbg_intr,5,"IE0");
            break;
        case VI_OE0:
            dbgPRINT(&dbg_intr,5,"OE0");
            break;
        case VI_TxDMA:
            dbgPRINT(&dbg_intr,5,"TxDMA");
            break;
        case VI_RxDMA:
            dbgPRINT(&dbg_intr,5,"RxDMA");
            break;
        case VI_HxDMA:
            dbgPRINT(&dbg_intr,5,"HxDMA");
            break;
        default:
            dbgPRINT(&dbg_intr,1,"unknown interrupt vector #%02x",val);
            if (val > VI_unkn) {
                val = VI_unkn;
            }
        }
    ...
    if (1 <= dbg_intr) {
        ...
        *mp++ = 0;
        if (!repeating || repeat_count <= 1) {
            dbgPRINT(&dbg_intr,1,"%u %c D#%02x M#%02x %s",icount,(repeating?'R':'-'),
    ...
    if (interrupt_count > 3000) {
        ...
        dbgPRINT(&dbg_intr,0,"Interrupt count too high for development SW, bailing out.");
...
static int install_cdsp_interrupt(bi_data *data)
{
    ...
    dbgPRINT(&dbg_init,1,"bi_data#%08x",(u32)(void*)data);
    b3 = JIMK_INT_STATUS;
    b4 = JIMK_INT_MASK;
#else
    //dbgPRINT(&dbg_init,1,"INTERS1@%08x INTERS2@%08x",
    ...
    dbgPRINT(&dbg_init,1,"inters1@%08x inters2@%08x bi_data#%08x",
    ...
    if (0 != request_irq(LNXINTNUM(AVALANCHE_USB_INT),cdsp_interrupt,
                         irq_flags,"CDSP",data)) {
        dbgPRINT(&dbg_init,1,"could not register USB interrupt.");
    ...
    //dbgPRINT(&dbg_init,1,"interrupt handler installed #%08x #%08x -> #%08x #%08x",b1,b2,a1,a2);
...
#ifdef JIMK_INT_CTRLR
    dbgPRINT(&dbg_init,1,"interrupt handler installed s#%08x m#%08x -> #%08x #%08x",
             b3,b4,JIMK_INT_STATUS,JIMK_INT_MASK);
    b3 = read_32bit_cp0_register(CP0_STATUS);
    dbgPRINT(&dbg_init,1,"MIPS CP0 status#%08x",b3);
    /* Fake an interrupt, to see if anything happens. */
    // 2000 Dec 29 - Victor Wells indicated that some of the interrupt logic
    //               is missing (for space reasons) from the RTL on the SEAD board,
    //               so this will not generate an interrupt or show the correct values.
    if (dbg_intr >= 5) {
        dbgPRINT(&dbg_intr,1,"about to test interrupt s#%08x m#%08x",JIMK_INT_STATUS,JIMK_INT_MASK);
        JIMK_INT_STATUS = 0xffffffff;
        dbgPRINT(&dbg_intr,1,"1st reading interrupt status and mask s#%08x m#%08x",
                 JIMK_INT_STATUS,JIMK_INT_MASK);
        JIMK_INT_STATUS = 0x1;
        dbgPRINT(&dbg_intr,1,"2nd reading interrupt status and mask s#%08x m#%08x",
                 JIMK_INT_STATUS,JIMK_INT_MASK);
        for (b3 = 0;  b3 < 10;  b3++) {
        ...
        dbgPRINT(&dbg_intr,1,"3rd reading interrupt status and mask s#%08x m#%08x",
                 JIMK_INT_STATUS,JIMK_INT_MASK);
    }
#else
    dbgPRINT(&dbg_init,5,"interrupt handler installed #%08x #%08x -> #%08lx #%08lx",
    ...
    //               is missing (for space reasons) from the RTL on the SEAD board,
    //               so this will not generate an interrupt or show the correct values.
    dbgPRINT(&dbg_intr,5,"about to test interrupt...");
    // icregs->intsr1 = (0x1 << AVALANCHE_USB_INT);
    icregs->intsr1 = 0xffffffff;
    dbgPRINT(&dbg_intr,5,"about to read raw interrupt status...");
    dbgPRINT(&dbg_intr,5,"raw interrupt status #%08lx",icregs->intcr1);
#endif
    return(0);
}
...
static void tick_tsklet(void *data)
{
    ...
    if (1 <= dbg_tick &&
        ...
        if (new && 3 <= dbg_tick) {
            dbgPRINT(&dbg_tick,1,"USB_Status #%02x->#%02x",old_USB_status,new_USB_status);
