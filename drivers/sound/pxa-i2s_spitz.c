...
#define WM87XX_init wm8750_init
#define WM87XX_close wm8750_close
#define WM87XX_open wm8750_open
#define WM87XX_busy wm8750_busy
#define WM87XX_set_output_volume wm8750_set_output_volume
#define WM87XX_set_freq wm8750_set_freq
#define WM87XX_set_mic_gain_boost wm8750_set_mic_gain_boost
#define WM87XX_suspend wm8750_suspend
#define WM87XX_resume wm8750_resume
...
static int audio_release(struct inode *inode, struct file *file)
{
	...
	err = WM87XX_close();
	if ( err ) {
	  PXAI2S_DBGPRINT(DBG_ALWAYS, "Can not close WM87XX\n");
	  return err;
	}
...
int audio_open(struct inode *inode, struct file *file )
{
	...
	// Open WM87XX1
	err = WM87XX_open(&settings);
	if ( err ) {
	  PXAI2S_DBGPRINT(DBG_ALWAYS, "Can not open WM87XX\n");
	  goto error;
	}
...
static void CorgiUpdateVolume(void)
{
  // If sound driver is opened , so update volume.
  if ( WM87XX_busy() ) {
    WM87XX_set_output_volume(sound.volume_left, sound.volume_right);
  }
}
...
{
  sound.gain       = sound.gain_left;
  if ( WM87XX_busy() ) {
    WM87XX_set_mic_gain_boost(sound.gain);
  }
}
...
static void CorgiInit(void)
{
  ...
  if ( request_irq(IRQ_GPIO_HP_IN, Corgi_hp_in_interrupt,
			...
			unsigned int cmd, unsigned long arg)
{
  ...
  case SOUND_MIXER_AGC:
    IOCTL_IN(arg, data);
    return IOCTL_OUT(arg, wm8750_set_agc(data));
#endif
...
static int __init pxa_i2s_init(void)
{
	...
	printk("Spitz audio driver initialize\n");
