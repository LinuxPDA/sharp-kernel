...
static struct map_info katana_map[] = {
	{
		...
		map_priv_1: MQ9000_FLASH_VBASE,
	},
	{
		name: "AM29LV640ML Flash (x16)",
		size: 0x00800000,
		buswidth: 2,
		read8: katana_x16_read8,
		read16: katana_x16_read16,
		read32: katana_x16_read32,
		copy_from: katana_x16_copy_from,
		write8: katana_x16_write8,
		write16: katana_x16_write16,
		write32: katana_x16_write32,
		copy_to: katana_x16_copy_to,
		map_priv_1: MQ9000_SSRAM_VBASE,
	}
};

static struct mtd_info *katana_mtd[2] = {NULL, NULL};
...
int __init init_katana(void)
{
	...
	for (i=0; i<2; i++) {
		for(; !katana_mtd[i] && *chip_type; chip_type++) {
			katana_mtd[i] = do_map_probe(*chip_type, &katana_map[i]);
		}
		if (!katana_mtd[i]) {
			return -ENXIO;
		}
	...
	katana_mtd[0]->module = THIS_MODULE;
	i = add_mtd_partitions(katana_mtd[0], katana_parts, PARTITION_COUNT);
	if (i != 0)
		return -ENXIO;
	katana_mtd[1]->module = THIS_MODULE;
	add_mtd_device(katana_mtd[1]);
...
static void __exit cleanup_katana(void)
{
	...
	for (i=0; i<2; i++) {
		del_mtd_device(katana_mtd[i]);
		map_destroy(katana_mtd[i]);
	}
}
