 ...
 * $Id: mtdram.c,v 1.28 2002/08/20 14:46:30 jonashg Exp $
...
#if CONFIG_TMTDRAM_TOTAL_SIZE > 0
    if (mtd_info->priv)
#if CONFIG_TMTDRAM_ABS_POS > 0
...
int __init init_mtdram(void)
{
  ...
  addr = ioremap(CONFIG_TMTDRAM_ABS_POS, MTDRAM_TOTAL_SIZE);
  if (!addr) {
...
#if !defined(CONFIG_ARCH_KATANA) && !defined(CONFIG_ARCH_DBMX1) && \
    !defined(CONFIG_ARCH_DBMX2)
  memset(mtd_info->priv, 0xff, MTDRAM_TOTAL_SIZE);
#endif
  return err;
}

#else /* CONFIG_TMTDRAM_ABS_POS > 0 */
...
#endif /* !(CONFIG_TMTDRAM_ABS_POS > 0) */

#else /* CONFIG_TMTDRAM_TOTAL_SIZE > 0 */

int __init init_mtdram(void)
{
  return 0;
}
#endif /* !(CONFIG_TMTDRAM_TOTAL_SIZE > 0) */
