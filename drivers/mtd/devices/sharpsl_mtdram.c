/*
 * sharpsl_mtdram - mtd device on RAM for SHARP SL series
 * Original Author: Alexander Larsson <alex@cendio.se>
 * Original $Id: mtdram.c,v 1.25 2001/10/02 15:05:13 dwmw2 Exp $
 *
...
#ifndef CONFIG_MTDRAM_SHARP_SL_ABS_POS
  #define CONFIG_MTDRAM_SHARP_SL_ABS_POS 0
#endif

#if CONFIG_MTDRAM_SHARP_SL_ABS_POS > 0
  #include <asm/io.h>
#endif

#ifdef MODULE
static unsigned long total_size = CONFIG_MTDRAM_SHARP_SL_TOTAL_SIZE;
static unsigned long erase_size = CONFIG_MTDRAM_SHARP_SL_ERASE_SIZE;
MODULE_PARM(total_size,"l");
MODULE_PARM(erase_size,"l");
#define MTDRAM_TOTAL_SIZE (total_size * 1024)
#define MTDRAM_ERASE_SIZE (erase_size * 1024)
#else
#define MTDRAM_TOTAL_SIZE (CONFIG_MTDRAM_SHARP_SL_TOTAL_SIZE * 1024)
#define MTDRAM_ERASE_SIZE (CONFIG_MTDRAM_SHARP_SL_ERASE_SIZE * 1024)
#endif
...
#if CONFIG_MTDRAM_SHARP_SL_ABS_POS > 0
   mtd_info->priv = ioremap(CONFIG_MTDRAM_SHARP_SL_ABS_POS, MTDRAM_TOTAL_SIZE);
#else
   mtd_info->priv = vmalloc(MTDRAM_TOTAL_SIZE);
#endif

   if (!mtd_info->priv) {
     DEBUG(MTD_DEBUG_LEVEL1, "Failed to vmalloc(/ioremap) memory region of size %ld (ABS_POS:%ld)\n", (long)MTDRAM_TOTAL_SIZE, (long)CONFIG_MTDRAM_SHARP_SL_ABS_POS);
     kfree(mtd_info);
     mtd_info = NULL;
     return -ENOMEM;
   }
#if !(CONFIG_MTDRAM_SHARP_SL_ABS_POS > 0) && defined(CONFIG_ARCH_SHARP_SL)
...
MODULE_DESCRIPTION("MTD driver for RAM");
