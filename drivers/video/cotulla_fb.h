...
#define ALLOCATED_DISCRIPTOR_MEM_SIZE \
	(PAGE_ALIGN(sizeof(LCD_DescriptorT) + PAGE_SIZE))
...
#if 0
typedef struct FrameBuffer_8bitS {
	unsigned short	palette[256];
	unsigned char	pixel[XRES][YRES];
} FrameBuffer_8bitT;
#endif

typedef struct FrameBuffer_16bitS {
	unsigned short	pixel[XRES * YRES];
} FrameBuffer_16bitT;
