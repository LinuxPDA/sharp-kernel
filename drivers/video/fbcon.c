 ...
 *   17-Oct-2002 SHARP
 *      generated interrupt of timer in exact timing with cursor blink.
 *   31-Oct-2002 SHARP
...
static void cursor_timer_handler(unsigned long dev_addr)
{
	  ...
	  if (cursor_on) {
		  if (vbl_cursor_cnt) {
			  cursor_timer.expires = jiffies+vbl_cursor_cnt*HZ/50;
		  ...
		  cursor_timer.expires = jiffies+60*60*HZ;
		  add_timer(&cursor_timer);
	  }
#else
      cursor_timer.expires = jiffies+HZ/50;
...
#if defined(CONFIG_FBCON_LOGO_CENTER)
	logo_lines = ((p->var.yres - LOGO_H + fontheight(p) - 1) / fontheight(p)) / 2 + ((LOGO_H + fontheight(p) - 1) / fontheight(p));
#else
...
#endif
#if defined(CONFIG_ARCH_SHARP_SL)
			mod_timer(&cursor_timer, jiffies + HZ/50);
#endif
            break;
        }
}
...
#endif
#if defined(CONFIG_SHARP_LOGO_SCREEN)
	if (fixed_logo) { // scroll only bottom line
		t = p->conp->vc_rows-1;
		b = p->conp->vc_rows;
		count = 1;
	}
...
#else
#if defined(CONFIG_FBCON_LOGO_CENTER)
    for (x = (p->var.xres - (LOGO_W + 8)) / 2;
	 x < (p->var.xres - (LOGO_W + 8)) / 2 + (LOGO_W + 8) &&
...
#else
#if defined(CONFIG_FBCON_LOGO_CENTER)
	    for( y1 = (p->var.yres - LOGO_H) / 2;
		 y1 < (p->var.yres - LOGO_H) / 2 + LOGO_H; y1++ ) {
#else
	    for( y1 = 0; y1 < LOGO_H; y1++ ) {
#endif
		dst = fb + y1*line + x*bdepth;
...
{
	/* logo=0 (scroll logo)  or logo=1 (fixed logo) */
	fixed_logo = simple_strtoul(str, NULL, 0);
//	printk("\n\nread logo=%d\n\n",fixed_logo);
	return 0;
}
