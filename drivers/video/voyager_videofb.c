...
static char voyafb_name[] = "voyager_video_fb";
			    ...
			    struct fb_info* info)
{
	...
	strcpy(fix->id, "voyager_video_fb");
...
static int __init voyafb_map_video_memory(void)
{
	if (vram_base != NULL)
		return -EINVAL;
        vram_base_phys = (u_char *)VOY_VRAM_TOP1;
...
//	*(volatile unsigned long *)(VIDEO_DISPLAY_CTRL) |= 0x3105;
//	*(volatile unsigned long *)(VIDEO_DISPLAY_CTRL) |= 0x01;
//	*(volatile unsigned long *)(VIDEO_DISPLAY_CTRL) |= 0x14005;
...
//	memset(vram_base_phys,0,size);
	memset((unsigned char *)VOY_VRAM_TOP1,0,size);
	memset((unsigned char *)VOY_VRAM_TOP2,0,size);
...
#ifdef DEBUG
		printk(KERN_ERR
		       "unable to register VOYAGER frame buffer video plane\n");
#endif
		return -EINVAL;
	}
	...
	printk("VOYAGER frame buffer video plane initialized.\n");
