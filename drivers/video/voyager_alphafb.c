...
static char voyafb_name[] = "voyager_alpha_fb";
			    ...
			    struct fb_info* info)
{
	...
	strcpy(fix->id, "voyager_alpha_fb");
...
static int __init voyafb_map_video_memory(void)
{
	if (vram_base != NULL)
		return -EINVAL;
        vram_base_phys = (u_char *)VOY_VRAM_TOP4;
...
//	*(volatile unsigned long *)(ALPHA_DISPLAY_CTRL) |= 0x05;
...
int	size;
...
#ifdef DEBUG
		printk(KERN_ERR
		       "unable to register VOYAGER frame buffer alpha plane\n");
#endif
		return -EINVAL;
	}
	...
	printk("VOYAGER frame buffer alpha plane initialized.\n");
